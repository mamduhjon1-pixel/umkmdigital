...
function CreateSubAdmin() {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!adminUser) throw new Error("Sesi admin tidak ditemukan. Silakan login ulang.");

      const cleanEmail = form.email.trim().toLowerCase();
      const cleanName = form.name.trim();
      const appName = `subadmin-create-${Date.now()}`;
      secondaryApp = initializeApp(firebaseConfig, appName);
      const secondaryAuth = getAuth(secondaryApp);

      const cred = await createUserWithEmailAndPassword(secondaryAuth, cleanEmail, form.password);
      ...
      alert(`Sub-admin berhasil dibuat: ${cleanEmail}`);
      setForm({ name: "", email: "", password: "" });
    } catch (err) {
      console.error("Gagal membuat sub-admin:", err);
      alert(err?.message || "Gagal membuat sub-admin");
    } finally {
      if (secondaryApp) {
  ...
  return (
    <div>
      ...
      <div className="card" style={{ maxWidth: 520 }}>
        <div style={{ background: "#ECFDF5", border: "1px solid #A7F3D0", borderRadius: 10, padding: 12, marginBottom: 16, fontSize: 13, color: "#065F46", lineHeight: 1.5 }}>
          Sub-admin dibuat otomatis dari dashboard. Akun admin utama tidak akan logout atau pindah session.
        </div>
        <form onSubmit={submit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div className="form-group">
          ...
          <div className="form-group">
            <label>Password</label>
            <input className="form-input" type="password" placeholder="Minimal 6 karakter" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={6} />
          </div>
          <button type="submit" className="btn-primary" disabled={loading}>{loading ? "Membuat..." : "Buat Sub-Admin"}</button>
        </form>
      </div>
    </div>
  );

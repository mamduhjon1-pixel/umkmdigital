...
const ADMIN_EMAILS = ["mamduhbadruzaman@gmail.com"];
...
function toTitleCaseLocation(value) {
  return String(value || "")
    .toLowerCase()
    .split(" ")
    .filter(Boolean)
...
function getDashboardRouteFromUrl() {
  try {
...
function getDefaultDashboardTab(roleOrPage) {
...
function pushAppRoute(page, tab = "") {
  try {
...
async function markDashboardNotificationsRead({ user, profile, role, types = [], extraFilter = null }) {
  if (!user?.uid) return;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Admin melihat semua notifikasi agar tidak ada notifikasi penting yang tersembunyi
    // karena variasi data lama: role=admin, role=chat, userId admin, atau notifikasi sistem lain.
  ...
  return (
    <div className={isAdminMode ? "app-shell admin-mode" : "app-shell"} style={{ minHeight: "100vh", background: isAdminMode ? "#F5F7FB" : "var(--bg)" }}>
      ...
      {page === "admin" && hasAdminAccess && (
        <AdminDashboard user={user} profile={{ ...profile, role: effectiveRole }} products={products} orders={sortOrdersByStage(orders)} withdrawals={withdrawals}
...
/* ─── BUYER DASHBOARD ────────────────────────── */
function BuyerDashboard({ user, profile, orders, products, paymentSetting, createNotif, onAddToCart, onProductClick, setPage, activeTab = "beranda", onTabChange, notifications = [], onLogout }) {
...
/* ─── SELLER DASHBOARD ───────────────────────── */
function SellerDashboard({ user, profile, products, orders, wallets, commissionBills = [], paymentSetting, commissionSetting, chatUnread = 0, createNotif, activeTab = "beranda", onTabChange, notifications = [], onLogout }) {
  const tab = activeTab || "beranda";
      ...
      <main className="seller-pro-main">
        ...
        {tab === "profil" && (
          <div className="seller-pro-panel seller-pro-profile-panel">
            <div className="seller-pro-panel-head"><div><b>Profil Toko</b><span>Informasi utama akun seller</span></div></div>
            <div className="seller-pro-profile-table">
              {[["Nama Toko", sellerName],["Email", profile?.email || user?.email || "-"],["WhatsApp", profile?.whatsapp || "-"],["Status", profile?.status === "active" ? "✅ Aktif" : "⏳ Pending"],["Saldo Tersedia", rupiah(wallet?.saldoTersedia || 0)],["Total Penjualan", rupiah(sellerTotalSales)]].map(([l,v]) => (
...
function AdminDashboard({ user, profile, products, orders, withdrawals, paymentSetting, manualBalance, commissionSetting, wallets, commissionBills = [], adminCommissionWallet, adminCommissionTransactions = [], users, createNotif, onLogout, setPage, activeTab = "order", onTabChange, notifications = [], unreadNotif = 0, unreadChat = 0 }) {
  ...
  const adminPrimaryStats = [
    { label: "Butuh Diproses", value: adminMenuBadges.order || 0, icon: "⚡", color: "#ee4d2d", tab: "order" },
    { label: "Seller Baru", value: adminMenuBadges.sellerApproval || 0, icon: "✅", color: "#2563eb", tab: "sellerApproval" },
    { label: "Chat Baru", value: unreadChat || 0, icon: "💬", color: "#16a34a", tab: "chat" },
    { label: "Komisi Pending", value: adminMenuBadges.commission || 0, icon: "💸", color: "#f59e0b", tab: "commission" },
  ];
  const adminSecondaryStats = [
    { label: "Produk", value: products.length, icon: "📦", color: "#6366f1", tab: "produk" },
  ...
  const adminQuickActions = [
    ...
    { id: "sellerApproval", label: "Seller", icon: "✅" },
    { id: "chat", label: "Chat", icon: "💬" },
    { id: "commission", label: "Komisi", icon: "💸" },
    ...(isAdmin ? [{ id: "produk", label: "Produk", icon: "📦" }, { id: "withdraw", label: "Tarik", icon: "💰" }] : []),
  ];

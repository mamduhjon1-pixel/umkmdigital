...
function RouteFallback() {
  return (
    <div className="route-skeleton" aria-label="Memuat halaman">
...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => {
    try { return localStorage.getItem("umkm_theme") || "light"; } catch { return "light"; }
  });

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", theme);
    try { localStorage.setItem("umkm_theme", theme); } catch {}
  }, [theme]);
      ...
      try {
        if (u) {
          ...
          if (snap.exists()) {
            ...
            const isCleanStart = window.location.pathname === "/" && (page === "login" || page === "home");
            if (isCleanStart) {
              try {
                const last = JSON.parse(localStorage.getItem("umkm_last_route") || "null");
                const allowed = last && (
                  (last.page === "seller" && detectedRole === "seller") ||
                  (last.page === "buyer" && detectedRole === "buyer") ||
                  (last.page === "admin" && ["admin", "sub_admin"].includes(detectedRole)) ||
                  (["home", "notif", "chat", "sellerStore"].includes(last.page))
                );
                if (allowed && localStorage.getItem("umkm_has_opened_before") === "yes") {
                  setPage(last.page);
                  setDashboardTab(last.tab || "");
                  pushAppRoute(last.page, last.tab || "");
                } else {
                  localStorage.setItem("umkm_has_opened_before", "yes");
                }
              } catch {}
            }
            ...
            try { localStorage.setItem("umkm_has_opened_before", "yes"); } catch {}
          } else {
            setProfile(null);
          }
        } else {
  ...
  const unreadNotif = visibleNotifications.filter((n) => !n.isRead && !n.read).length;
  const unreadChat = chats.filter((chat) => {
    if (!user?.uid || chat.lastSenderId === user.uid) return false;
    const readAt = chat.readBy?.[user.uid];
    return getMillis(chat.lastMessageAt || chat.updatedAt) > getMillis(readAt);
  }).length;
  ...
  const isDashboardMode = user && ((page === "admin" && hasAdminAccess) || (page === "seller" && effectiveRole === "seller") || (page === "buyer" && effectiveRole === "buyer"));
  const isAdminMode = isDashboardMode;

  useEffect(() => {
    if (!user || !profile) return;
    if (page === "admin" && !hasAdminAccess) navGoTo("home");
    if (page === "seller" && effectiveRole !== "seller") navGoTo("home");
    if (page === "buyer" && effectiveRole !== "buyer") navGoTo("home");
  }, [user?.uid, profile?.role, page, effectiveRole, hasAdminAccess]);
  ...
  function navGoTo(p, tab = "") {
    ...
    try {
      if (user && !["login", "register"].includes(p)) {
        localStorage.setItem("umkm_last_route", JSON.stringify({ page: p, tab: finalTab, at: Date.now() }));
        localStorage.setItem("umkm_has_opened_before", "yes");
      }
    } catch {}
    scrollToTopSmooth();
  ...
  return (
    <div className={isAdminMode ? "app-shell admin-mode" : "app-shell"} style={{ minHeight: "100vh", background: isAdminMode ? "#F5F7FB" : "var(--bg)" }}>
      ...
      <InstallAppPrompt />
      {/* ── DESKTOP NAVBAR ── */}
      {!isDashboardMode && <div className="nav-sticky nav-desktop">
        <div style={{ background: "var(--orange)" }}>
          <div className="nav-inner">
            ...
            <div className="nav-actions">
              <button className="nav-icon-btn" onClick={toggleTheme} title="Mode gelap/terang">
                {theme === "dark" ? "☀️" : "🌙"}
              </button>
              {user && (
      ...
      {/* ── MOBILE NAVBAR ── */}
      {!isDashboardMode && <div className="nav-sticky nav-mobile marketplace-mobile-header">
        <div className="nav-mobile-top">
          <span className="nav-mobile-logo app-logo-wrap" onClick={() => navGoTo("home")}><img src="/icon-192.png" alt="UMKM Digital" /><b>UMKM</b></span>
          <div className="nav-mobile-icons">
            <button className="nav-icon-btn" onClick={toggleTheme} style={{ fontSize: 18, padding: "4px 6px" }} title="Mode gelap/terang">{theme === "dark" ? "☀️" : "🌙"}</button>
            {user && (
            ...
            {user && (
              <button className="nav-icon-btn" onClick={() => navGoTo("chat")} style={{ fontSize: 20, padding: "4px 6px" }} title="Chat">
                💬{unreadChat > 0 && <span className="badge-count">{unreadChat}</span>}
              </button>
            )}
            {user && (
      ...
      {page === "buyer" && effectiveRole === "buyer" && (
        <Suspense fallback={<RouteFallback />}><BuyerDashboard user={user} profile={profile} orders={sortOrdersByStage(orders.filter((o) => o.buyerId === user.uid))}
          ...
          activeTab={dashboardTab || "beranda"} onTabChange={(tab) => navDashboard("buyer", tab)} notifications={visibleNotifications}
      ...
      {page === "seller" && effectiveRole === "seller" && (
        <Suspense fallback={<RouteFallback />}><SellerDashboard user={user} profile={profile}
          ...
          activeTab={dashboardTab || "beranda"} onTabChange={(tab) => navDashboard("seller", tab)} notifications={visibleNotifications}
          onLogout={logoutAndClearSession} /></Suspense>
      )}
      {page === "admin" && hasAdminAccess && (
        <Suspense fallback={<RouteFallback />}><AdminDashboard user={user} profile={{ ...profile, role: effectiveRole }} products={products} orders={sortOrdersByStage(orders)} withdrawals={withdrawals}
          ...
          activeTab={dashboardTab || "order"} onTabChange={(tab) => navDashboard("admin", tab)} notifications={visibleNotifications} unreadNotif={unreadNotif} unreadChat={unreadChat} /></Suspense>
      )}
      ...
      {page === "notif" && user && (
        <NotificationPage notifications={visibleNotifications} />
      )}
      {page === "chat" && user && (

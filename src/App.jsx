    ...
    <div className={isAdminMode ? "app-shell admin-mode" : "app-shell"} style={{ minHeight: "100vh", background: isAdminMode ? "#F5F7FB" : "var(--bg)" }}>
      ...
      {page === "admin" && hasAdminAccess && (
        <AdminDashboard user={user} profile={{ ...profile, role: effectiveRole }} products={products} orders={sortOrdersByStage(orders)} withdrawals={withdrawals}
          ...
          users={allUsers} createNotif={createNotif} onLogout={logoutAndClearSession} setPage={navGoTo} unreadNotif={unreadNotif} unreadChat={unreadChat} />
      )}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 14, marginBottom: 28 }}>
              {[
                ...
                <div key={s.label} className="admin-pro-stat-card">
...
function SellerDashboard({ user, profile, products, orders, wallets, commissionBills = [], paymentSetting, commissionSetting, chatUnread = 0, createNotif, onLogout }) {
  ...
  const quickStats = [
    { label: "Produk Aktif", value: activeProducts, sub: `${products.length} total produk`, icon: "📦", color: "#2563EB" },
    { label: "Pesanan Aktif", value: activeOrders, sub: `${completedOrders} selesai`, icon: "🛒", color: "#7C3AED" },
    { label: "Saldo Tersedia", value: rupiah(wallet?.saldoTersedia || 0), sub: "Siap diajukan withdraw", icon: "💰", color: "#059669" },
    { label: "Tagihan Komisi", value: rupiah(commissionDebt), sub: commissionDebt > 0 ? "Perlu diselesaikan" : "Tidak ada tagihan", icon: "💸", color: commissionDebt > 0 ? "#DC2626" : "#10B981" },
    { label: "Total Penjualan", value: rupiah(sellerTotalSales), sub: "Order selesai", icon: "📈", color: "#EA580C" },
    { label: "Total Stok", value: totalStock, sub: "Stok tersedia", icon: "🏷️", color: "#0891B2" },
  ];
          ...
          {tabs.map((t) => {
            ...
            return (
              <button key={t.id} type="button" className={`seller-pro-menu-item ${tab === t.id ? "active" : ""}`} onClick={() => goTab(t.id)} disabled={blockedOrderTab}>
                ...
                <span>{t.label}</span>
                {t.id === "order" && !blockedOrderTab && sellerIncomingOrderCount > 0 && <span className="seller-pro-badge">{sellerIncomingOrderCount}</span>}
                {t.id === "chat" && chatUnread > 0 && <span className="seller-pro-badge">{chatUnread}</span>}
      ...
      <main className="seller-pro-main">
        <div className="seller-pro-topbar">
          ...
          <div className="seller-pro-top-actions">
            <button type="button" className="seller-pro-action" onClick={() => setTab("produk")}>+ Tambah Produk</button>
        ...
        {tab === "beranda" && (
          <div>
            ...
            <section className="seller-pro-grid-two">
              ...
              <div className="seller-pro-panel">
                <div className="seller-pro-panel-head"><div><b>Produk Toko</b><span>Produk terbaru dan stok saat ini</span></div><button type="button" onClick={() => setTab("produk")}>Kelola Produk</button></div>
                <div className="seller-pro-list">
...
function AdminDashboard({ user, profile, products, orders, withdrawals, paymentSetting, manualBalance, commissionSetting, wallets, commissionBills = [], adminCommissionWallet, adminCommissionTransactions = [], users, createNotif, onLogout, setPage, unreadNotif = 0, unreadChat = 0 }) {
    ...
    <div className="admin-pro-shell">
      <aside className="admin-pro-sidebar">
        ...
        <div className="admin-pro-menu">
          {tabs.map((t) => (
            <button key={t.id} type="button" className={`admin-pro-menu-item ${tab === t.id ? "active" : ""}`} onClick={() => setTab(t.id)}>
      ...
      <main className="admin-pro-main">
        <div className="admin-pro-topbar">
          <div>
            <div className="admin-pro-page-kicker">Dashboard</div>
            <h1>Kontrol Admin</h1>
            <p>Kelola order, seller, produk, komisi, saldo, dan aktivitas penting dari satu tempat.</p>
          </div>
          <div className="admin-pro-actions">
            <button type="button" onClick={() => setPage && setPage("notif")} className="admin-pro-action-btn">🔔 Notifikasi {unreadNotif > 0 ? <span>{unreadNotif}</span> : null}</button>
            <button type="button" onClick={() => setTab("chat")} className="admin-pro-action-btn">💬 Chat {unreadChat > 0 ? <span>{unreadChat}</span> : null}</button>
          </div>
        </div>
        <div className="admin-pro-stats">
          {[

...
import { scrollToTopSmooth, getDashboardRouteFromUrl, getDefaultDashboardTab, getPathForRoute, pushAppRoute } from "./utils/navigationUtils";
...
export default function App() {
  ...
  const [buyerSellerUnreadChatCount, setBuyerSellerUnreadChatCount] = useState(0);
  const [buyerBanners, setBuyerBanners] = useState([]);


  useEffect(() => {
    const bannerRef = doc(db, "app_settings", "buyer_banners");
    const unsub = onSnapshot(bannerRef, (snap) => {
      ...
      console.error("Buyer banners realtime error:", error);
      setBuyerBanners([]);
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    const onPopState = () => {
      ...
      const loggedInRole = normalizeRole(profile?.role || profile?.peran, user?.email || profile?.email);

      if (user && ["login", "register"].includes(nextPage)) {
        const dashboardPage = loggedInRole === "seller" ? "seller" : loggedInRole === "buyer" ? "buyer" : (loggedInRole === "admin" || loggedInRole === "sub_admin") ? "admin" : "home";
        const dashboardTab = getDefaultDashboardTab(dashboardPage);
        setPage(dashboardPage);
        setDashboardTab(dashboardTab);
        try { window.history.replaceState({ page: dashboardPage, tab: dashboardTab }, "", getPathForRoute(dashboardPage, dashboardTab)); } catch {}
      } else {
        setPage(nextPage);
      ...
      try {
        if (u) {
          ...
          if (snap.exists()) {
            ...
            let freshNavigation = null;
            try {
              freshNavigation = JSON.parse(sessionStorage.getItem("umkm_fresh_navigation") || "null");
            } catch {}
            const isFreshPublicNavigation = freshNavigation
              && ["home", "sellerStore"].includes(freshNavigation.page)
              && freshNavigation.page === page
              && Date.now() - Number(freshNavigation.at || 0) < 30000;

            if (isFreshPublicNavigation) {
              try { sessionStorage.removeItem("umkm_fresh_navigation"); } catch {}
            } else if (!currentPage && (page === "login" || page === "home")) {
              const dashboardPage = (detectedRole === "admin" || detectedRole === "sub_admin") ? "admin" : detectedRole === "seller" ? "seller" : detectedRole === "buyer" ? "buyer" : "home";
              const dashboardTab = getDefaultDashboardTab(dashboardPage);
              setPage(dashboardPage);
              setDashboardTab(dashboardTab);
              if (page === "login") {
            ...
            if (isFreshPublicNavigation) {
              try { sessionStorage.removeItem("umkm_fresh_navigation"); } catch {}
            } else if (page === "login" || page === "home") {
              setPage("admin");
              setDashboardTab(getDefaultDashboardTab("admin"));
              if (page === "login") {
                try { window.history.replaceState({ page: "admin", tab: getDefaultDashboardTab("admin") }, "", getPathForRoute("admin", getDefaultDashboardTab("admin"))); } catch {}
              }
            }
          } else {
            setProfile(null);
    ...
    setCart((prev) => {
      ...
      if (exists) {
        return prev.map((i) => {
          ...
          const nextQty = Math.min(i.quantity + 1, stock);
  ...
  function buyNow(product, quantity = 1) {
    ...
    const stock = getStock(product);
    if (isOutOfStock(product) || safeQty > stock) {
      alert("Stok produk tidak mencukupi.");
      return;
    }
    setBuyNowItem({ ...product, quantity: safeQty, isBuyNow: true });
    setSelectedProduct(null);
    setShowCart(false);
  ...
  function updateQty(id, qty) {
    ...
    setCart((prev) => prev.map((i) => {
      if (i.id !== id) return i;
      const live = products.find((p) => p.id === id) || i;
      const stock = getStock(live);
      const safeQty = Math.min(Math.max(1, Number(qty || 1)), Math.max(stock, 1));
      return { ...i, ...live, quantity: safeQty };
    }));
  }
  ...
  const activeSellerIds = new Set(
    allUsers
      .filter(isActiveSellerAccount)
      .map((seller) => seller.uid || seller.id)
      .filter(Boolean)
  );
  const activeProducts = products.filter((p) =>
    ...
    && !p.isDeleted
    && Boolean(p.sellerId)
    && activeSellerIds.has(p.sellerId)
  );
  ...
  const hideGlobalHeader = page === "buyer" || page === "seller";

  // Pages that admin is allowed to visit without being redirected back to admin dashboard
  const ADMIN_ALLOWED_PAGES = ["admin", "notif", "chat", "sellerStore", "home"];

  function isAdminSellerChatRecord(chat) {
  ...
  const publicHeaderChatUnread = ["buyer", "seller"].includes(effectiveRole) ? buyerSellerUnreadChatCount : unreadChat;

  useEffect(() => {
    if (!user?.uid) {
      setUnreadChatCount(0);
      setBuyerSellerUnreadChatCount(0);
      return;
    }
    const qUnreadChats = query(collection(db, "chats"), where("unreadFor", "==", user.uid));
    const unsub = onSnapshot(qUnreadChats, (snap) => {
      const unreadChats = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
      setUnreadChatCount(unreadChats.length);
      setBuyerSellerUnreadChatCount(unreadChats.filter((chat) => !isAdminSellerChatRecord(chat)).length);
    }, (error) => {
      console.error("Unread chat badge error:", error);
      setUnreadChatCount(0);
      setBuyerSellerUnreadChatCount(0);
    });
    return () => unsub();
  ...
  useEffect(() => {
    if (user && isKnownAdminEmail(user.email) && !ADMIN_ALLOWED_PAGES.includes(page)) {
      setPage("admin");
      setShowCart(false);
      setSelectedProduct(null);
    }
  ...
  function navGoTo(p, tab = "") {
    ...
    const buyerNeedsFreshReload = effectiveRole === "buyer"
      && ((page !== "buyer" && p === "buyer") || (page === "buyer" && ["home", "sellerStore"].includes(p)));

    const sellerNeedsFreshReload = effectiveRole === "seller"
      && ((page !== "seller" && p === "seller") || (page === "seller" && ["home", "sellerStore"].includes(p)));

    setShowCart(false);
    setSelectedProduct(null);

    if (buyerNeedsFreshReload || sellerNeedsFreshReload) {
      const nextPath = getPathForRoute(p, finalTab);
      try {
        sessionStorage.setItem("umkm_fresh_navigation", JSON.stringify({ role: effectiveRole, page: p, tab: finalTab, at: Date.now() }));
      } catch {}
  ...
  return (
    ...
    <div className={isAdminMode ? "app-shell admin-mode" : "app-shell"} style={{ minHeight: "100vh", background: isAdminMode ? "#F5F7FB" : "var(--bg)" }}>
      ...
      <InstallAppPrompt />
      {/* ── DESKTOP NAVBAR ── */}
      {!hideGlobalHeader && <div className="nav-sticky nav-desktop">
        <div style={{ background: "var(--orange)" }}>
          <div className="nav-inner">
            <div className="nav-logo" onClick={() => navGoTo("home")}>UMKM<span>Digital</span></div>
            <div className="nav-search">
              <input placeholder="Cari produk, kategori, toko..." value={search}
            ...
            <div className="nav-actions">
              ...
              {user && (
                <button className="nav-icon-btn" onClick={() => navGoTo("chat")} title="Chat">
                  💬{publicHeaderChatUnread > 0 && <span className="badge-count">{publicHeaderChatUnread > 99 ? "99+" : publicHeaderChatUnread}</span>}
                </button>
              )}
              {!user ? (
                <>
      ...
      {/* ── MOBILE NAVBAR ── */}
      {!hideGlobalHeader && <div className="nav-sticky nav-mobile">
        <div className="nav-mobile-top">
          ...
          <div className="nav-mobile-icons">
            ...
            {user && (
              <button className="nav-icon-btn" onClick={() => navGoTo("chat")} title="Chat" style={{ fontSize: 20, padding: "4px 6px" }}>
                💬{publicHeaderChatUnread > 0 && <span className="badge-count">{publicHeaderChatUnread > 99 ? "99+" : publicHeaderChatUnread}</span>}
              </button>
            )}
            {!user && (
      ...
      {showCheckout && (
        <Suspense fallback={<RouteFallback />}><CheckoutModal
          cart={buyNowItem ? [buyNowItem] : selectedCartItems}
          user={user}
          profile={profile}
          onClose={() => { setShowCheckout(false); setBuyNowItem(null); }}
          onSuccess={() => {
            if (!buyNowItem) {
      ...
      {page === "buyer" && effectiveRole === "buyer" && (
        <Suspense fallback={<RouteFallback />}><BuyerDashboard user={user} profile={profile} orders={sortOrdersByStage(orders.filter((o) => o.buyerId === user.uid))}
          ...
          chatUnread={unreadChat} buyerBanners={buyerBanners}
      ...
      {page === "seller" && effectiveRole === "seller" && (
        <Suspense fallback={<RouteFallback />}><SellerDashboard user={user} profile={profile}
          ...
          wallets={wallets} withdrawals={withdrawals} commissionBills={commissionBills} paymentSetting={paymentSetting} commissionSetting={commissionSetting} chatUnread={unreadChat} createNotif={createNotif}
      ...
      {page === "admin" && hasAdminAccess && (
        <Suspense fallback={<RouteFallback />}><AdminDashboard user={user} profile={{ ...profile, role: effectiveRole }} products={products} orders={sortOrdersByStage(orders)} withdrawals={withdrawals}
          ...
          activeTab={dashboardTab || "order"} onTabChange={(tab) => navDashboard("admin", tab)} notifications={notifications} unreadNotif={unreadNotif} unreadChat={unreadChat}
          buyerBanners={buyerBanners} /></Suspense>
      )}
      ...
      {page === "notif" && user && (
        <NotificationPage notifications={notifications} setPage={navGoTo} profile={{ ...profile, role: effectiveRole }} />
      )}
      {page === "chat" && user && (
        <Suspense fallback={<RouteFallback />}><ChatCenter user={user} profile={profile} createNotif={createNotif} mode={effectiveRole === "admin" || effectiveRole === "sub_admin" ? "admin" : "buyer"} users={allUsers} /></Suspense>
      )}
      ...
      {/* Bottom navigation dihapus sesuai permintaan agar tidak muncul di semua halaman. */}
    </div>
    </ToastProvider>
  );
}

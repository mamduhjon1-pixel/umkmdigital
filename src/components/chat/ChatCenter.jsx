  ...
  async function sendMessage(e) {
    ...
    try {
      ...
      await addDoc(collection(db, "chats", activeChat.id, "messages"), {
        ...
        senderId: user.uid,
        senderName: isAdminRole ? "Admin" : (profile?.name || "User"),
      ...
      await setDoc(doc(db, "chats", activeChat.id), {
        ...
        updatedAt: serverTimestamp(),
        [`readBy.${user.uid}`]: serverTimestamp(),
        [`unreadFor.${receiverId || "unknown"}`]: true,
      }, { merge: true });
      // Chat dan notifikasi dipisah total: pesan chat tidak lagi membuat dokumen notifications.
      setText("");
    } catch (err) {
          ...
          {chats.length === 0 ? (
            ...
            return (
              <button key={c.id} onClick={() => setActiveChat(c)}
                ...
                <div style={{ fontWeight: 800, fontSize: 14 }}>{otherName}{isAdminSellerChat(c) && otherName === "Admin" && <span className="admin-chat-badge">Admin</span>}</div>
            ...
            <>
              <div style={{ padding: 14, borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                <div style={{ fontWeight: 800 }}>
                  {getOtherName(activeChat)}{isAdminSellerChat(activeChat) && getOtherName(activeChat) === "Admin" && <span className="admin-chat-badge">Admin</span>}

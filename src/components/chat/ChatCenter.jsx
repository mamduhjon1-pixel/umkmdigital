...
export default function ChatCenter({ user, profile, createNotif, mode = "buyer", users = [] }) {
  ...
  useEffect(() => {
    ...
    if (activeChat.unreadFor === user.uid) {
      updateDoc(doc(db, "chats", activeChat.id), {
        unreadFor: null,
        lastReadBy: user.uid,
        lastReadAt: serverTimestamp(),
      }).catch((error) => console.warn("Gagal menandai chat sudah dibaca:", error));
    }
  }, [activeChat?.id, activeChat?.unreadFor, user?.uid, isAdminRole]);

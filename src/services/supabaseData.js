...
export function onSnapshot(ref, cb, errCb) {
  ...
  const scheduleEmit = (delay = 160) => {
    if (!active) return;
    if (timer) clearTimeout(timer);
    timer = setTimeout(() => { timer = null; emit(); }, delay);
  };
  emit();
  const channel = supabase.channel(`rt_${ref.path.join("_")}_${randomId()}`)
    .on("postgres_changes", { event: "*", schema: "public", table: tableFromPath(ref.path) }, () => scheduleEmit(160))
    .subscribe();
  return () => {
    active = false;
    if (timer) clearTimeout(timer);

...
export async function creditSellerBalanceOnce(orderId, orderData, createNotif) {
  ...
  const walletRef = doc(db, "seller_wallets", orderData.sellerId);
  const orderRef = doc(db, "orders", orderId);
  const txRef = doc(db, "wallet_transactions", `transfer_order_completed_credit_${orderId}`);

  await runTransaction(db, async (transaction) => {
    const orderSnap = await transaction.get(orderRef);
    if (!orderSnap.exists()) return;
    const fresh = orderSnap.data() || {};
    if (fresh.balanceCredited === true) return;
...
const CANCELLED_ORDER_STATUSES = new Set(["dibatalkan", "ditolak"]);

function isOrderCancelledOrRejected(status) {
  return CANCELLED_ORDER_STATUSES.has(String(status || "").toLowerCase());
}

function getVoucherCollectionName(voucherType) {
  return voucherType === "seller" ? "seller_vouchers" : "marketplace_vouchers";
}

async function tryRollbackVoucherUsageInTransaction(transaction, orderData, voucherType) {
  const usageId = voucherType === "seller"
    ? orderData.sellerVoucherUsageId
    : orderData.marketplaceVoucherUsageId;
  const voucherId = voucherType === "seller"
    ? orderData.sellerVoucherId
    : orderData.marketplaceVoucherId;
  const discount = voucherType === "seller"
    ? Number(orderData.sellerVoucherDiscount || 0)
    : Number(orderData.marketplaceVoucherDiscount || orderData.voucherDiscountAmount || 0);
  const rolledBackField = voucherType === "seller" ? "sellerVoucherRolledBack" : "marketplaceVoucherRolledBack";

  if (!usageId || !voucherId || discount <= 0 || orderData[rolledBackField] === true) return;

  const usageRef = doc(db, "voucher_usages", usageId);
  const usageSnap = await transaction.get(usageRef);
  if (!usageSnap.exists()) return;

  const usage = usageSnap.data() || {};
  if (usage.voucherRolledBack === true) return;

  const orderIds = Array.isArray(usage.orderIds) ? usage.orderIds : [];
  const cancelledOrderIds = new Set([...(usage.cancelledOrderIds || []), orderData.id]);
  let allTerminal = orderIds.length > 0;

  for (const orderId of orderIds) {
    if (cancelledOrderIds.has(orderId)) continue;
    const relatedSnap = await transaction.get(doc(db, "orders", orderId));
    if (!relatedSnap.exists()) {
      cancelledOrderIds.add(orderId);
      continue;
    }
    const related = relatedSnap.data() || {};
    if (!isOrderCancelledOrRejected(related.statusPesanan)) {
      allTerminal = false;
      break;
    }
  }
  ...
  transaction.update(usageRef, {
    cancelledOrderIds: Array.from(cancelledOrderIds),
    updatedAt: serverTimestamp(),
  });

  if (!allTerminal) return;

  const voucherRef = doc(db, getVoucherCollectionName(voucherType), voucherId);
  const buyerId = orderData.buyerId || usage.buyerId || "";
  const voucherUpdate = {
    usedCount: increment(-1),
    updatedAt: serverTimestamp(),
  };
  if (buyerId) voucherUpdate[`usedBy.${buyerId}`] = increment(-1);
  transaction.update(voucherRef, voucherUpdate);
  transaction.update(usageRef, {
...
export async function completeOrderAndCreditSeller(order, extra = {}, createNotif) {
  ...
  const productRef = order.productId ? doc(db, "products", order.productId) : null;
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (!orderSnap.exists()) return;
    const fresh = orderSnap.data() || {};
    if (fresh.soldCounted === true) return;
  ...
  const completedOrderData = { ...order, statusPesanan: "selesai", soldCounted: true, ...extra };
  ...
  await autoPayOpenCommissionBillsFromSellerBalance(
    order.sellerId,
    createNotif,
    isTransferPayment(completedOrderData) ? "Saldo order transfer/QRIS masuk" : "Cek saldo setelah pesanan selesai"
  );
}
...
export async function restoreProductStockOnce(order) {
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (!fresh.stockRestored && fresh.productId && qty > 0) {
      transaction.set(doc(db, "products", fresh.productId), {
        stock: increment(qty),
        updatedAt: serverTimestamp(),
      }, { merge: true });
      transaction.update(orderRef, {
    ...
    await tryRollbackVoucherUsageInTransaction(transaction, fresh, "marketplace");
    await tryRollbackVoucherUsageInTransaction(transaction, fresh, "seller");

    transaction.update(orderRef, {
      marketplaceVoucherRolledBack: fresh.marketplaceVoucherId ? true : fresh.marketplaceVoucherRolledBack,
      sellerVoucherRolledBack: fresh.sellerVoucherId ? true : fresh.sellerVoucherRolledBack,
      updatedAt: serverTimestamp(),
    });
  });
}

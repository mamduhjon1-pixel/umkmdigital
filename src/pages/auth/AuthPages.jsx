...
export function LoginPage({ setPage }) {
  ...
  async function login(e) {
    ...
    try {
      ...
      // Setelah Firebase Auth berhasil, auth listener utama akan membaca role dari Firestore
      // lalu mengarahkan user ke dashboard yang tepat. Loading dibiarkan aktif agar UX terasa jelas.
    } catch (err) {
    ...
    <div className="auth-container" style={{ minHeight: "calc(100vh - 110px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div className="card form-card-mobile" style={{ padding: 36 }}>
          ...
          {loading && (
            <div className="auth-progress-card" role="status" aria-live="polite">
              <div className="auth-progress-spinner" />
              <div>
                <strong>Memverifikasi akun...</strong>
                <span>Menyiapkan dashboard UMKMDigital Anda</span>
              </div>
            </div>
          )}
          <form onSubmit={login} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <button className="btn-primary auth-submit-btn" style={{ width: "100%", justifyContent: "center", padding: 13, fontSize: 15 }} disabled={loading}>
              {loading ? <><span className="auth-btn-dot" /> Masuk...</> : "Masuk"}
            </button>
          </form>

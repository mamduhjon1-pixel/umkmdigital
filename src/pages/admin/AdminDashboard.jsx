...
export default function AdminDashboard({ user, profile, products, orders, withdrawals, paymentSetting, manualBalance, commissionSetting, wallets, commissionBills = [], adminCommissionWallet, adminCommissionTransactions = [], users, createNotif, onLogout, setPage, activeTab = "order", onTabChange, notifications = [], unreadNotif = 0, unreadChat = 0, buyerBanners = [] }) {
  ...
  const tabs = [
    ...
    ...(isAdmin ? [
      ...
      { id: "buyerBanners", label: "Banner Buyer", icon: "🖼️" },
      { id: "marketplaceVouchers", label: "Voucher", icon: "🎟️" },
  ...
  const adminMenuBadges = {
    ...
    buyerBanners: Math.max(0, 5 - buyerBanners.length),
    marketplaceVouchers: 0,
  ...
  return (
    <div className="admin-pro-shell">
      ...
      <main className="admin-pro-main">
        ...
        {tab === "buyerBanners" && isAdmin && <AdminBuyerBanners banners={buyerBanners} />}
        {tab === "marketplaceVouchers" && isAdmin && <AdminMarketplaceVouchers user={user} />}
...
function AdminMarketplaceVouchers({ user }) {
  const emptyForm = {
    name: "",
    code: "",
  ...
  const loadVouchers = async () => {
    ...
    try {
      ...
      alert(error?.message || "Gagal memuat voucher marketplace.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const getVoucherStatus = (voucher) => {
    ...
    if (voucher.status !== "active") return { label: "Nonaktif", className: "badge-red" };
    if (start && now < start) return { label: "Belum Aktif", className: "badge-yellow" };
    if (end && now > end) return { label: "Expired", className: "badge-red" };
    if (quota > 0 && used >= quota) return { label: "Habis", className: "badge-red" };
    return { label: "Aktif", className: "badge-green" };
  };
  ...
  const handleEdit = (voucher) => {
    ...
    setForm({
      ...
      limitPerUser: String(voucher.limitPerUser || 1),
      startAt: voucher.startAt || "",
      endAt: voucher.endAt || "",
      status: voucher.status || "active",
      paymentTransfer: (voucher.paymentMethods || []).includes("transfer"),
      paymentQris: (voucher.paymentMethods || []).includes("qris"),
      paymentCash: (voucher.paymentMethods || []).includes("cash"),
      targetScope: voucher.targetScope || "all",
      targetCategories: (voucher.targetCategories || []).join(", "),
      targetSellerIds: (voucher.targetSellerIds || []).join(", "),
      note: voucher.note || "",
    });
  ...
  const saveVoucher = async (event) => {
    ...
    const paymentMethods = [
      form.paymentTransfer ? "transfer" : null,
      form.paymentQris ? "qris" : null,
      form.paymentCash ? "cash" : null,
    ].filter(Boolean);

    if (!form.name.trim()) return alert("Nama promo wajib diisi.");
    if (!code) return alert("Kode voucher wajib diisi.");
    if (numberValue(form.discountValue) <= 0) return alert("Nilai diskon wajib lebih dari 0.");
    if (paymentMethods.length === 0) return alert("Pilih minimal satu metode pembayaran.");
    if (form.endAt && form.startAt && new Date(form.endAt).getTime() <= new Date(form.startAt).getTime()) return alert("Waktu berakhir harus setelah waktu mulai.");
    ...
    try {
      const payload = {
        ...
        minPurchase: numberValue(form.minPurchase),
        maxDiscount: numberValue(form.maxDiscount),
        quota: numberValue(form.quota),
        usedCount: editingId ? Number(vouchers.find((item) => item.id === editingId)?.usedCount || 0) : 0,
        limitPerUser: Math.max(1, Number(form.limitPerUser || 1)),
        startAt: form.startAt || "",
        endAt: form.endAt || "",
        status: form.status,
        paymentMethods,
        targetScope: form.targetScope,
        targetCategories: splitList(form.targetCategories),
        targetSellerIds: splitList(form.targetSellerIds),
        note: form.note.trim(),
        updatedAt: serverTimestamp(),
      ...
      if (editingId) {
        await updateDoc(doc(db, "marketplace_vouchers", editingId), payload);
      } else {
        await addDoc(collection(db, "marketplace_vouchers"), {
          ...payload,
          createdAt: serverTimestamp(),
      ...
      await loadVouchers();
      resetForm();
      alert(editingId ? "Voucher berhasil diperbarui." : "Voucher marketplace berhasil dibuat.");
    } catch (error) {
  ...
  const toggleVoucherStatus = async (voucher) => {
    ...
    try {
      await updateDoc(doc(db, "marketplace_vouchers", voucher.id), {
        status: nextStatus,
        updatedAt: serverTimestamp(),
        updatedBy: user?.uid || "admin",
      });
      ...
      alert(error?.message || "Gagal mengubah status voucher.");
    }
  };

  const activeVouchers = vouchers.filter((voucher) => getVoucherStatus(voucher).label === "Aktif").length;
  const usedTotal = vouchers.reduce((sum, voucher) => sum + Number(voucher.usedCount || 0), 0);
  const quotaTotal = vouchers.reduce((sum, voucher) => sum + Number(voucher.quota || 0), 0);
    ...
    <div className="admin-voucher-page">
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "flex-start" }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 900, marginBottom: 6 }}>🎟️ Voucher Marketplace</div>
            <p style={{ margin: 0, color: "var(--text2)", fontSize: 13, lineHeight: 1.6 }}>
              Voucher marketplace sudah terhubung ke checkout. Kuota, waktu aktif, metode pembayaran, dan limit penggunaan akan divalidasi otomatis saat buyer memakai voucher.
            </p>
          </div>
          <button type="button" className="btn-ghost btn-sm" onClick={loadVouchers} disabled={loading}>{loading ? "Memuat..." : "Refresh"}</button>
        </div>
      </div>

      <div className="admin-pro-stats" style={{ marginBottom: 16 }}>
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#F9731615" }}><span>🎟️</span></div><div style={{ fontWeight: 900, color: "var(--orange)", fontSize: 22 }}>{vouchers.length}</div><div className="stat-label">Total Voucher</div></div>
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#10B98115" }}><span>✅</span></div><div style={{ fontWeight: 900, color: "#10B981", fontSize: 22 }}>{activeVouchers}</div><div className="stat-label">Aktif</div></div>
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#3B82F615" }}><span>📦</span></div><div style={{ fontWeight: 900, color: "#3B82F6", fontSize: 22 }}>{quotaTotal || "∞"}</div><div className="stat-label">Total Kuota</div></div>
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#8B5CF615" }}><span>🧾</span></div><div style={{ fontWeight: 900, color: "#8B5CF6", fontSize: 22 }}>{usedTotal}</div><div className="stat-label">Terpakai</div></div>
      </div>

      <form className="card" onSubmit={saveVoucher} style={{ marginBottom: 18 }}>
        <h3 style={{ marginTop: 0 }}>{editingId ? "Edit Voucher Marketplace" : "Buat Voucher Marketplace"}</h3>
        <div className="form-grid">
          <input value={form.name} onChange={(e) => updateForm("name", e.target.value)} placeholder="Nama promo, contoh: Promo Awal Bulan" />
          <input value={form.code} onChange={(e) => updateForm("code", cleanCode(e.target.value))} placeholder="Kode voucher, contoh: HEMAT10" />
          <select value={form.discountType} onChange={(e) => updateForm("discountType", e.target.value)}>
            <option value="nominal">Diskon Nominal Rupiah</option>
            <option value="percent">Diskon Persentase</option>
          </select>
          <input type="number" min="0" value={form.discountValue} onChange={(e) => updateForm("discountValue", e.target.value)} placeholder={form.discountType === "percent" ? "Nilai %, contoh: 10" : "Nilai diskon, contoh: 10000"} />
          <input type="number" min="0" value={form.minPurchase} onChange={(e) => updateForm("minPurchase", e.target.value)} placeholder="Minimal belanja, contoh: 50000" />
          <input type="number" min="0" value={form.maxDiscount} onChange={(e) => updateForm("maxDiscount", e.target.value)} placeholder="Maksimal diskon, kosongkan jika tidak ada" />
          <input type="number" min="0" value={form.quota} onChange={(e) => updateForm("quota", e.target.value)} placeholder="Kuota voucher, kosong/0 = unlimited" />
          <input type="number" min="1" value={form.limitPerUser} onChange={(e) => updateForm("limitPerUser", e.target.value)} placeholder="Limit per akun buyer" />
          <label style={{ fontSize: 12, color: "var(--text2)" }}>Mulai aktif<input type="datetime-local" value={form.startAt} onChange={(e) => updateForm("startAt", e.target.value)} /></label>
          <label style={{ fontSize: 12, color: "var(--text2)" }}>Berakhir<input type="datetime-local" value={form.endAt} onChange={(e) => updateForm("endAt", e.target.value)} /></label>
          <select value={form.status} onChange={(e) => updateForm("status", e.target.value)}>
            <option value="active">Aktif</option>
            <option value="inactive">Nonaktif</option>
          </select>
          <select value={form.targetScope} onChange={(e) => updateForm("targetScope", e.target.value)}>
            <option value="all">Berlaku untuk semua produk</option>
            <option value="category">Kategori tertentu</option>
            <option value="seller">Seller tertentu</option>
          </select>
          <input value={form.targetCategories} onChange={(e) => updateForm("targetCategories", e.target.value)} placeholder="Kategori target, pisahkan koma. Opsional" />
          <input value={form.targetSellerIds} onChange={(e) => updateForm("targetSellerIds", e.target.value)} placeholder="ID seller target, pisahkan koma. Opsional" />
        </div>
        <div className="voucher-payment-options">
          <label><input type="checkbox" checked={form.paymentTransfer} onChange={(e) => updateForm("paymentTransfer", e.target.checked)} /> Transfer</label>
          <label><input type="checkbox" checked={form.paymentQris} onChange={(e) => updateForm("paymentQris", e.target.checked)} /> QRIS</label>
          <label><input type="checkbox" checked={form.paymentCash} onChange={(e) => updateForm("paymentCash", e.target.checked)} /> Tunai/COD</label>
        </div>
        <textarea value={form.note} onChange={(e) => updateForm("note", e.target.value)} placeholder="Catatan internal admin. Opsional" rows="3" />
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
          <button className="btn-primary" type="submit" disabled={saving}>{saving ? "Menyimpan..." : editingId ? "Simpan Perubahan" : "Buat Voucher"}</button>
          {editingId && <button className="btn-ghost" type="button" onClick={resetForm}>Batal Edit</button>}
        </div>
      </form>

      {loading ? (
        <div className="empty-state"><div className="empty-icon">⏳</div><p>Memuat voucher...</p></div>
      ) : vouchers.length === 0 ? (
        <div className="empty-state"><div className="empty-icon">🎟️</div><p>Belum ada voucher marketplace</p></div>
      ) : (
        <div className="admin-voucher-list">
          {vouchers.map((voucher) => {
            ...
            return (
              <div className="card admin-voucher-card" key={voucher.id}>
                <div className="admin-voucher-card-head">
                  <div>
                    <div className="admin-voucher-code">{voucher.code}</div>
                    <b>{voucher.name}</b>
                    <div style={{ color: "var(--text2)", fontSize: 12, marginTop: 4 }}>{voucher.discountType === "percent" ? `${voucher.discountValue}%` : rupiah(voucher.discountValue)} • Min. {rupiah(voucher.minPurchase || 0)}</div>
                  </div>
                  <span className={`badge ${status.className}`}>{status.label}</span>
                </div>
                <div className="admin-voucher-meta">
                  <span>Kuota: {quota > 0 ? quota : "Unlimited"}</span>
                  <span>Terpakai: {used}</span>
                  <span>Sisa: {remaining}</span>
                  <span>Limit/User: {voucher.limitPerUser || 1}x</span>
                  <span>Bayar: {(voucher.paymentMethods || []).join(", ") || "-"}</span>
                  <span>Mulai: {voucher.startAt ? new Date(voucher.startAt).toLocaleString("id-ID") : "Sekarang"}</span>
                  <span>Akhir: {voucher.endAt ? new Date(voucher.endAt).toLocaleString("id-ID") : "Tanpa batas"}</span>
                </div>
                {voucher.note && <div style={{ marginTop: 10, fontSize: 12, color: "var(--text2)", background: "#F8FAFC", borderRadius: 8, padding: 8 }}>{voucher.note}</div>}
                <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                  <button className="btn-primary btn-sm" type="button" onClick={() => handleEdit(voucher)}>Edit</button>
                  <button className="btn-ghost btn-sm" type="button" onClick={() => toggleVoucherStatus(voucher)}>{voucher.status === "active" ? "Nonaktifkan" : "Aktifkan"}</button>
                </div>
              </div>
            );
          })}
...
function AdminBuyerBanners({ banners = [] }) {
  const [uploading, setUploading] = useState(false);
  const [localTitle, setLocalTitle] = useState("");
  const maxBanner = 5;
  const sortedBanners = (Array.isArray(banners) ? banners : [])
    .filter((item) => item?.imageUrl)
    .slice(0, maxBanner)
    .sort((a, b) => Number(a.order || 0) - Number(b.order || 0));

  async function saveBanners(nextBanners) {
    await setDoc(doc(db, "app_settings", "buyer_banners"), {
      banners: nextBanners.slice(0, maxBanner).map((item, index) => ({
  ...
  async function handleUpload(event) {
    ...
    const remaining = maxBanner - sortedBanners.length;
    if (remaining <= 0) {
      alert("Maksimal 5 banner. Hapus banner lama sebelum upload baru.");
      return;
    }
    const selectedFiles = files.slice(0, remaining);
    if (files.length > remaining) {
      alert(`Hanya ${remaining} banner lagi yang bisa ditambahkan. File sisanya diabaikan.`);
    }
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        uploaded.push({
          ...
          imageUrl,
          title: localTitle.trim() || `Banner ${sortedBanners.length + uploaded.length + 1}`,
          order: sortedBanners.length + uploaded.length,
          createdAt: new Date().toISOString(),
        });
      }
      await saveBanners([...sortedBanners, ...uploaded]);
      setLocalTitle("");
      alert("Banner berhasil diupload.");
    } catch (error) {
      alert(error.message || "Upload banner gagal.");
    } finally {
      setUploading(false);
    }
  }
  ...
  async function removeBanner(index) {
    ...
    await saveBanners(sortedBanners.filter((_, i) => i !== index));
  }
  ...
  return (
    <div className="admin-buyer-banner-page">
      <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>🖼️ Banner Iklan Buyer</div>
      <p style={{ color: "var(--text2)", fontSize: 13, lineHeight: 1.6, marginBottom: 16 }}>
        Upload maksimal 5 foto banner. Gambar disimpan di Cloudinary, lalu URL banner disimpan di Firestore untuk tampil di dashboard buyer.
      </p>

      <div className="card admin-banner-upload-card">
        <div>
          <b>Upload Banner</b>
          <span>Format JPG, PNG, WEBP. Maksimal mengikuti limit upload Cloudinary saat ini: 1MB per foto.</span>
        </div>
        <div className="admin-banner-upload-actions">
          <input
            ...
            disabled={uploading}
          />
          <label className={`btn-primary ${uploading || sortedBanners.length >= maxBanner ? "disabled" : ""}`}>
            {uploading ? "Mengupload..." : `Upload Foto (${sortedBanners.length}/${maxBanner})`}
            <input
              type="file"
              accept="image/jpeg,image/png,image/webp"
              multiple
              hidden
              disabled={uploading || sortedBanners.length >= maxBanner}
      ...
      {sortedBanners.length === 0 ? (
        <div className="empty-state">
          ...
          <p>Belum ada banner. Dashboard buyer akan menampilkan placeholder.</p>
        </div>
      ) : (
        <div className="admin-banner-grid">
          {sortedBanners.map((banner, index) => (
            <div className="admin-banner-item" key={banner.id || banner.imageUrl}>
              <img src={banner.imageUrl} alt={banner.title || `Banner ${index + 1}`} onClick={() => openImagePreview(banner.imageUrl)} />
              <div className="admin-banner-item-body">
                <b>{banner.title || `Banner ${index + 1}`}</b>
                <span>Urutan {index + 1}</span>
                <div className="admin-banner-item-actions">
                  <button className="btn-ghost btn-sm" onClick={() => moveBanner(index, -1)} disabled={index === 0}>↑</button>
                  <button className="btn-ghost btn-sm" onClick={() => moveBanner(index, 1)} disabled={index === sortedBanners.length - 1}>↓</button>
                  <button className="btn-ghost btn-sm" style={{ color: "#ef4444", borderColor: "#fecaca" }} onClick={() => removeBanner(index)}>Hapus</button>
                </div>
              </div>
            </div>
          ))}
...
function AdminOrders({ orders, createNotif }) {
  ...
  async function reject(o) {
    ...
    await restoreProductStockOnce({ ...o, statusPembayaran: "ditolak", statusPesanan: "dibatalkan" });
  ...
  async function approveCancel(o) {
    ...
    await restoreProductStockOnce({ ...o, statusPesanan: "dibatalkan", cancelStatus: "approved" });
    await cancelCashCommissionBillForOrder(o.id);

...
export default function CheckoutModal({ cart, user, profile, onClose, onSuccess, createNotif }) {
  ...
  const [form, setForm] = useState({
    ...
    buyerMapsLink: savedAddress.buyerMapsLink || profile?.mapsLink || "",
    ...
    addressNote: savedAddress.addressNote || profile?.addressNote || "",
  });
    ...
    try {
      ...
      try {
        await setDoc(doc(db, "users", user.uid), {
          name: form.buyerName || profile?.name || "",
          whatsapp: form.buyerWhatsapp || profile?.whatsapp || "",
          fullAddress: form.buyerAddress || "",
          detailAddress: form.buyerAddress || "",
          mapsLink: form.buyerMapsLink || "",
        ...
        console.error("Gagal menyimpan profil/alamat checkout otomatis:", error);
      }
      ...
      for (const rawItem of cart) {
        ...
        const orderPayload = {
          ...
          buyerUsername: String(profile?.username || profile?.buyerUsername || ""),
          buyerPhotoURL: String(profile?.photoURL || profile?.buyerPhotoURL || ""),
          ...
          buyerAddressNote: String(form.addressNote || ""),
  ...
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal" style={{ maxWidth: 600 }} onClick={(e) => e.stopPropagation()}>
      ...
      <form onSubmit={handleCheckout}><div className="modal-body checkout-body">
        ...
        <div className="form-group"><label>Catatan Alamat</label><textarea className="form-input" rows={2} value={form.addressNote} onChange={(e) => setForm({ ...form, addressNote: e.target.value })} placeholder="Contoh: rumah pagar biru, dekat masjid" /></div>

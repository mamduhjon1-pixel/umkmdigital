...
import { addDoc, collection, doc, getDocs, increment, query, serverTimestamp, updateDoc, where, writeBatch } from "firebase/firestore";
...
import { paymentMethodLabel, isCashPayment, isTransferPayment, getSellerReceivableAmount, getProductCommissionTotal, getCashMarketplaceVoucherCompensationAmount, getCashCommissionDueAmount } from "../../utils/paymentUtils";
...
export default function SellerDashboard({ user, profile, products, orders, withdrawals = [], wallets, commissionBills = [], paymentSetting, commissionSetting, chatUnread = 0, createNotif, activeTab = "beranda", onTabChange, notifications = [], setPage, onLogout }) {
  ...
  const tabs = [
    ...
    { id: "voucher", label: "Voucher Toko", icon: "🎟️" },
  ...
  return (
    <div className="seller-pro-shell">
      ...
      <main className="seller-pro-main">
        ...
        {tab === "voucher" && <SellerVouchers user={user} profile={profile} products={products.filter((p) => !p.isDeleted)} />}
...
function SellerVouchers({ user, profile, products = [] }) {
  const emptyForm = {
    name: "",
    code: "",
  ...
  async function loadSellerVouchers() {
    ...
    setLoading(true);
    try {
      const snap = await getDocs(query(collection(db, "seller_vouchers"), where("sellerId", "==", user.uid)));
      const items = snap.docs.map((item) => ({ id: item.id, ...(item.data() || {}) }))
        .sort((a, b) => String(a.code || "").localeCompare(String(b.code || "")));
      setVouchers(items);
    } catch (error) {
      console.error("Gagal memuat voucher seller:", error);
      alert(error?.message || "Gagal memuat voucher toko.");
    } finally {
      setLoading(false);
    }
  }

  useEffect(() => { loadSellerVouchers(); }, [user?.uid]);
  ...
  function voucherStatus(voucher) {
    ...
    if (voucher.status !== "active") return { label: "Nonaktif", cls: "danger" };
    if (start && now < start) return { label: "Terjadwal", cls: "warning" };
    if (end && now > end) return { label: "Expired", cls: "danger" };
    if (quota > 0 && used >= quota) return { label: "Habis", cls: "danger" };
    return { label: "Aktif", cls: "success" };
  }
  ...
  function editVoucher(voucher) {
    ...
    setForm({
      ...
      targetProductIds: voucher.targetProductIds || [],
      note: voucher.note || "",
    });
  ...
  async function submitVoucher(e) {
    ...
    if (!Number(form.discountValue || 0) || Number(form.discountValue || 0) <= 0) return alert("Nilai diskon wajib lebih dari 0.");
    if (form.targetScope === "product" && (!form.targetProductIds || form.targetProductIds.length === 0)) return alert("Pilih minimal satu produk untuk voucher produk tertentu.");

    const payload = {
      type: "seller",
      owner: "seller",
      sellerId: user.uid,
      sellerName: profile?.storeName || profile?.namaToko || profile?.name || "Seller",
      ...
      targetScope: form.targetScope || "all",
      targetProductIds: form.targetScope === "product" ? form.targetProductIds : [],
      note: form.note || "",
      updatedAt: serverTimestamp(),
    };
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "seller_vouchers", editingId), payload);
      } else {
        await addDoc(collection(db, "seller_vouchers"), {
          ...payload,
          usedCount: 0,
          usedBy: {},
      ...
      alert("Voucher toko berhasil disimpan.");
    } catch (error) {
      console.error("Gagal menyimpan voucher toko:", error);
      alert(error?.message || "Gagal menyimpan voucher toko.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function toggleVoucher(voucher) {
    try {
      await updateDoc(doc(db, "seller_vouchers", voucher.id), {
  ...
  const selectedProducts = products.filter((p) => (form.targetProductIds || []).includes(p.id));

  return (
    <div className="admin-voucher-page seller-voucher-page">
      <div className="admin-pro-section-head">
        <div>
          <h2>🎟️ Voucher Toko</h2>
          <p>Promo ini ditanggung seller sendiri. Komisi marketplace dihitung dari harga setelah promo seller.</p>
        </div>
      </div>

      <div className="admin-pro-stat-grid" style={{ marginBottom: 16 }}>
        ...
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#10B98115" }}><span>✅</span></div><div style={{ fontWeight: 900, color: "#10B981", fontSize: 22 }}>{vouchers.filter((v) => voucherStatus(v).label === "Aktif").length}</div><div className="stat-label">Aktif</div></div>
        <div className="admin-pro-stat-card"><div className="stat-icon" style={{ background: "#2563EB15" }}><span>📈</span></div><div style={{ fontWeight: 900, color: "#2563EB", fontSize: 22 }}>{vouchers.reduce((s, v) => s + Number(v.usedCount || 0), 0)}</div><div className="stat-label">Terpakai</div></div>
      </div>

      <div className="card" style={{ marginBottom: 18 }}>
        <div style={{ fontSize: 16, fontWeight: 900, marginBottom: 12 }}>{editingId ? "Edit Voucher Toko" : "Buat Voucher Toko"}</div>
        <form onSubmit={submitVoucher} className="form-grid">
          <label>Nama Promo<input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Contoh: Promo Buah Segar" /></label>
          <label>Kode Voucher<input className="form-input" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase().replace(/\s+/g, "") })} placeholder="TOKOADIN10" required /></label>
          <label>Tipe Diskon<select className="form-input" value={form.discountType} onChange={(e) => setForm({ ...form, discountType: e.target.value })}><option value="nominal">Nominal Rupiah</option><option value="percent">Persen</option></select></label>
          <label>Nilai Diskon<input className="form-input" type="number" value={form.discountValue} onChange={(e) => setForm({ ...form, discountValue: e.target.value })} placeholder="10000 / 10" required /></label>
          <label>Minimal Belanja<input className="form-input" type="number" value={form.minPurchase} onChange={(e) => setForm({ ...form, minPurchase: e.target.value })} placeholder="50000" /></label>
          <label>Maksimal Diskon<input className="form-input" type="number" value={form.maxDiscount} onChange={(e) => setForm({ ...form, maxDiscount: e.target.value })} placeholder="Khusus diskon persen" /></label>
          <label>Kuota Voucher<input className="form-input" type="number" value={form.quota} onChange={(e) => setForm({ ...form, quota: e.target.value })} placeholder="0 = unlimited" /></label>
          <label>Limit per Buyer<input className="form-input" type="number" value={form.limitPerUser} onChange={(e) => setForm({ ...form, limitPerUser: e.target.value })} min="1" /></label>
          <label>Mulai Aktif<input className="form-input" type="datetime-local" value={form.startAt} onChange={(e) => setForm({ ...form, startAt: e.target.value })} /></label>
          <label>Berakhir<input className="form-input" type="datetime-local" value={form.endAt} onChange={(e) => setForm({ ...form, endAt: e.target.value })} /></label>
          <label>Status<select className="form-input" value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="active">Aktif</option><option value="inactive">Nonaktif</option></select></label>
          <label>Berlaku Untuk<select className="form-input" value={form.targetScope} onChange={(e) => setForm({ ...form, targetScope: e.target.value, targetProductIds: [] })}><option value="all">Semua produk toko</option><option value="product">Produk tertentu</option></select></label>
          {form.targetScope === "product" && <div className="form-group" style={{ gridColumn: "1/-1" }}>
            <label>Pilih Produk Promo</label>
            <div className="voucher-product-picker">
              {products.length === 0 ? <div style={{ color: "var(--text3)", fontSize: 13 }}>Belum ada produk aktif.</div> : products.map((p) => {
                const checked = (form.targetProductIds || []).includes(p.id);
                return <label key={p.id} className="voucher-product-option"><input type="checkbox" checked={checked} onChange={(e) => setForm({ ...form, targetProductIds: e.target.checked ? [...(form.targetProductIds || []), p.id] : (form.targetProductIds || []).filter((id) => id !== p.id) })} /><span>{p.productName}</span><small>{rupiah(p.price || 0)}</small></label>;
              })}
            </div>
            {selectedProducts.length > 0 && <small style={{ color: "var(--text3)" }}>{selectedProducts.length} produk dipilih.</small>}
          </div>}
          <label style={{ gridColumn: "1/-1" }}>Catatan<textarea className="form-input" rows="2" value={form.note} onChange={(e) => setForm({ ...form, note: e.target.value })} placeholder="Opsional, contoh: Promo khusus akhir pekan" /></label>
          <div style={{ gridColumn: "1/-1", display: "flex", gap: 10, justifyContent: "flex-end" }}>
            {editingId && <button type="button" className="btn-ghost" onClick={resetForm}>Batal Edit</button>}
            <button type="submit" className="btn-primary" disabled={loading}>{loading ? "Menyimpan..." : editingId ? "Simpan Perubahan" : "Buat Voucher"}</button>
          </div>
        </form>
      </div>

      <div className="admin-voucher-list">
        {vouchers.length === 0 ? <div className="empty-state"><div className="empty-icon">🎟️</div><p>Belum ada voucher toko.</p></div> : vouchers.map((voucher) => {
          const status = voucherStatus(voucher);
          const remaining = Number(voucher.quota || 0) > 0 ? Math.max(0, Number(voucher.quota || 0) - Number(voucher.usedCount || 0)) : "Unlimited";
          return <article key={voucher.id} className="admin-voucher-card">
            <div className="admin-voucher-card-head"><div><span className="admin-voucher-code">{voucher.code}</span><b>{voucher.name || voucher.code}</b></div><span className={`badge ${status.cls}`}>{status.label}</span></div>
            <div className="admin-voucher-meta">
              <span>{voucher.discountType === "percent" ? `Diskon ${voucher.discountValue}%` : `Diskon ${rupiah(voucher.discountValue)}`}</span>
              <span>Min. {rupiah(voucher.minPurchase || 0)}</span>
              <span>Sisa {remaining}</span>
              <span>{voucher.targetScope === "product" ? `${(voucher.targetProductIds || []).length} produk` : "Semua produk"}</span>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
              <button type="button" className="btn-ghost btn-sm" onClick={() => editVoucher(voucher)}>Edit</button>
              <button type="button" className="btn-ghost btn-sm" onClick={() => toggleVoucher(voucher)}>{voucher.status === "active" ? "Nonaktifkan" : "Aktifkan"}</button>
            </div>
          </article>;
        })}
      </div>
...
function SellerOrders({ orders, createNotif, hasCommissionDebt = false, commissionDebt = 0 }) {
  ...
  async function quoteShipping(o) {
    ...
    const sellerDiscountedProductTotal = Math.max(0, Number(o.sellerDiscountedProductTotal || o.commissionBase || productTotal) || productTotal);
    const marketplaceVoucherDiscount = Math.max(0, Number(o.marketplaceVoucherDiscount || o.voucherDiscountAmount || 0));
    const adminFee = getProductCommissionTotal(o);
    const sellerAmount = Math.max(0, sellerDiscountedProductTotal - adminFee + cost);
    const totalAmount = Math.max(0, sellerDiscountedProductTotal - marketplaceVoucherDiscount + cost);
    const cashSettlementPreview = { ...o, sellerAmount, totalAmount, cashReceivedBySeller: totalAmount };

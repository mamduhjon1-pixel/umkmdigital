...
/* ─── SELLER DASHBOARD ───────────────────────── */
export default function SellerDashboard({ user, profile, products, orders, wallets, commissionBills = [], paymentSetting, commissionSetting, chatUnread = 0, createNotif, activeTab = "beranda", onTabChange, notifications = [], onLogout }) {
  ...
  const topProduct = products
    ...
    .map((p) => ({ ...p, soldCount: orders.filter((o) => o.productId === p.id && o.statusPesanan === "selesai").reduce((sum, o) => sum + Number(o.quantity || 1), 0) }))
    .sort((a, b) => b.soldCount - a.soldCount)[0];
  useEffect(() => {
  ...
  const quickStats = [
    ...
    { label: "Komisi", value: rupiah(commissionDebt), sub: commissionDebt > 0 ? "Perlu bayar" : "Aman", icon: "🧾", color: commissionDebt > 0 ? "#DC2626" : "#10B981" },
  ];
    ...
    <div className="seller-pro-shell">
      <aside className="seller-pro-sidebar">
        <div className="seller-pro-brand">
          <div className="seller-pro-logo">S</div>
          <div>
            <div className="seller-pro-brand-title">Seller Centre</div>
            <div className="seller-pro-brand-sub">UMKM Jampang Surade</div>
          </div>
        </div>
          ...
          {tabs.map((t) => {
            ...
            return (
              <button key={t.id} type="button" className={`seller-pro-menu-item ${tab === t.id ? "active" : ""}`} onClick={() => goTab(t.id)} disabled={blockedOrderTab}>
                ...
                {t.id === "chat" && tab !== "chat" && chatUnread > 0 && <span className="seller-pro-badge">{chatUnread}</span>}
      ...
      <main className="seller-pro-main">
        ...
        {tab === "beranda" && (
          <div>
            ...
            <section className="seller-mobile-shortcuts" aria-label="Aksi cepat seller">
              <button type="button" onClick={() => goTab("order")}><b>📋 Order</b><span>{sellerIncomingOrderCount} baru</span></button>
              <button type="button" onClick={() => goTab("produk")}><b>➕ Produk</b><span>Tambah/kelola</span></button>
              <button type="button" onClick={() => goTab("chat")}><b>💬 Chat</b><span>{chatUnread} belum dibaca</span></button>
              <button type="button" onClick={() => goTab("withdraw")}><b>💳 Saldo</b><span>{rupiah(wallet?.saldoTersedia || 0)}</span></button>
            </section>
            ...
            <div className="seller-compact-analytics">
              <div><span>Produk terlaris</span><b>{topProduct?.soldCount > 0 ? (topProduct.productName || topProduct.name || "Produk") : "Belum ada"}</b></div>
              <div><span>Terjual</span><b>{topProduct?.soldCount || 0} pcs</b></div>
              <div><span>Order selesai</span><b>{completedOrders}</b></div>
              <div><span>Stok total</span><b>{totalStock}</b></div>
            </div>
        ...
        {tab === "profil" && (
          <SellerStoreProfile user={user} profile={profile} wallet={wallet} sellerTotalSales={sellerTotalSales} />
        )}
      </main>
    </div>
  );
...
function SellerStoreProfile({ user, profile, wallet, sellerTotalSales }) {
  const [form, setForm] = useState({
    name: profile?.name || "",
    username: profile?.username || profile?.storeUsername || "",
    storeDescription: profile?.storeDescription || profile?.description || "",
    whatsapp: profile?.whatsapp || "",
    address: profile?.address || profile?.sellerAddress || "",
    instagram: profile?.instagram || profile?.socialInstagram || "",
    facebook: profile?.facebook || profile?.socialFacebook || "",
    tiktok: profile?.tiktok || profile?.socialTiktok || "",
    isStoreOpen: profile?.isStoreOpen !== false,
  });
  const [photoFile, setPhotoFile] = useState(null);
  const [bannerFile, setBannerFile] = useState(null);
  const [saving, setSaving] = useState(false);
  const storePhoto = profile?.storePhoto || profile?.photoURL || profile?.avatarUrl || "";
  const storeBanner = profile?.storeBanner || profile?.bannerUrl || "";
  ...
  async function submit(e) {
    ...
    try {
      const updates = {
        ...
        storeUsername: form.username.trim(),
        storeDescription: form.storeDescription.trim(),
        whatsapp: form.whatsapp.trim(),
        address: form.address.trim(),
        sellerAddress: form.address.trim(),
        instagram: form.instagram.trim(),
        socialInstagram: form.instagram.trim(),
        facebook: form.facebook.trim(),
        socialFacebook: form.facebook.trim(),
        tiktok: form.tiktok.trim(),
        socialTiktok: form.tiktok.trim(),
        isStoreOpen: Boolean(form.isStoreOpen),
        updatedAt: serverTimestamp(),
      };
      if (photoFile) updates.storePhoto = await uploadImageToCloudinary(photoFile);
      if (bannerFile) updates.storeBanner = await uploadImageToCloudinary(bannerFile);
      ...
      alert(error?.message || "Gagal menyimpan profil toko. Coba lagi.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="seller-pro-panel seller-pro-profile-panel seller-store-profile-editor">
      <div className="seller-pro-panel-head"><div><b>Profil Toko</b><span>Kelola identitas toko yang dilihat pembeli.</span></div></div>
      <div className="seller-store-preview">
        <div className="seller-store-banner" style={{ backgroundImage: storeBanner ? `url(${storeBanner})` : "linear-gradient(135deg,#ff6b35,#ffb703)" }} />
        <div className="seller-store-preview-row">
          <div className="seller-store-photo">{storePhoto ? <img src={storePhoto} alt="Foto toko" /> : (form.name?.[0]?.toUpperCase() || "T")}</div>
          <div>
            <h3>{form.name || "Nama Toko"}</h3>
            <p>@{form.username || "username_toko"} • {form.isStoreOpen ? "Buka" : "Tutup"}</p>
          </div>
        </div>
      </div>
      <form onSubmit={submit} className="seller-store-form">
        <div className="form-group"><label>Foto Toko</label><input className="form-input" type="file" accept="image/*" onChange={(e) => setPhotoFile(e.target.files?.[0] || null)} /></div>
        <div className="form-group"><label>Banner Toko</label><input className="form-input" type="file" accept="image/*" onChange={(e) => setBannerFile(e.target.files?.[0] || null)} /></div>
        <div className="form-group"><label>Nama Toko</label><input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
        <div className="form-group"><label>Username Toko</label><input className="form-input" placeholder="contoh: toko_jampang" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} /></div>
        <div className="form-group full"><label>Deskripsi Toko</label><textarea className="form-input" rows={3} value={form.storeDescription} onChange={(e) => setForm({ ...form, storeDescription: e.target.value })} /></div>
        ...
        <div className="form-group"><label>Status Toko</label><select className="form-input" value={form.isStoreOpen ? "open" : "closed"} onChange={(e) => setForm({ ...form, isStoreOpen: e.target.value === "open" })}><option value="open">Buka</option><option value="closed">Tutup</option></select></div>
        <div className="form-group full"><label>Alamat Toko</label><textarea className="form-input" rows={2} value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} /></div>
        <div className="form-group"><label>Instagram</label><input className="form-input" value={form.instagram} onChange={(e) => setForm({ ...form, instagram: e.target.value })} /></div>
        <div className="form-group"><label>Facebook</label><input className="form-input" value={form.facebook} onChange={(e) => setForm({ ...form, facebook: e.target.value })} /></div>
        <div className="form-group"><label>TikTok</label><input className="form-input" value={form.tiktok} onChange={(e) => setForm({ ...form, tiktok: e.target.value })} /></div>
        <div className="seller-pro-profile-table full">
          {[["Email", profile?.email || user?.email || "-"],["Status Akun", profile?.status === "active" ? "✅ Aktif" : "⏳ Pending"],["Saldo Tersedia", rupiah(wallet?.saldoTersedia || 0)],["Total Penjualan", rupiah(sellerTotalSales)]].map(([l,v]) => (
            <div key={l}><span>{l}</span><b>{v}</b></div>
          ))}
        </div>
        <button type="submit" className="btn-primary full" disabled={saving}>{saving ? "Menyimpan..." : "Simpan Profil Toko"}</button>
      </form>
    </div>
  );
}
            ...
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 8, padding: 10, background: "#F8FAFC", border: "1px solid var(--border)", borderRadius: 10 }}>
                  ...
                  {o.buyerAddressNote && <div>Catatan: {o.buyerAddressNote}</div>}

...
/* ─── BUYER DASHBOARD ────────────────────────── */
export default function BuyerDashboard({ user, profile, orders, products, paymentSetting, createNotif, onAddToCart, onProductClick, setPage, activeTab = "beranda", onTabChange, notifications = [], chatUnread = 0, buyerBanners = [], onLogout }) {
  ...
  const tabs = [
    { id: "home", label: "Home", short: "Home", icon: "🏠", isMarketplace: true },
    { id: "pesanan", label: "Pesanan", short: "Pesanan", icon: "📦", badge: buyerActiveOrderCount },
    { id: "belanja", label: "Belanja", short: "Belanja", icon: "🔎", isMarketplace: true },
    { id: "chat", label: "Pesan", short: "Pesan", icon: "💬", badge: chatUnread },
  ...
  return (
    <div className="buyer-app-shell">
      <aside className="buyer-app-nav" aria-label="Menu pembeli">
        <button type="button" className="buyer-app-brand buyer-app-brand-button" onClick={() => setPage("home")} aria-label="Kembali ke halaman utama publik">
          <div className="buyer-app-logo">🛍️</div>
          <div>
            <div className="buyer-app-brand-title">UMKMDigital</div>
            <div className="buyer-app-brand-sub">Marketplace publik</div>
          </div>
        </button>
        ...
        <nav className="buyer-app-menu">
          {tabs.map((t) => (
            <button
              key={t.id}
              type="button"
              className={`buyer-app-menu-item ${t.isMarketplace ? "buyer-app-menu-marketplace" : ""} ${tab === t.id ? "active" : ""}`}
              onClick={() => t.isMarketplace ? setPage("home") : goTab(t.id)}
              aria-label={t.isMarketplace ? "Kembali ke halaman marketplace" : t.label}
            >
      ...
      <main className="buyer-app-main">
        <div className="buyer-mobile-header">
          <button type="button" className="buyer-mobile-brand-button" onClick={() => setPage("home")} aria-label="Kembali ke halaman utama publik">
            <div className="buyer-mobile-kicker">UMKMDigital</div>
            <div className="buyer-mobile-title">Marketplace</div>
          </button>
        ...
        {tab === "beranda" && (
          <div className="buyer-home-page">
            ...
            <BuyerAdCarousel banners={buyerBanners} onExplore={() => setPage("home")} />
        ...
        {tab === "chat" && <div className="buyer-page-card"><ChatCenter user={user} profile={profile} createNotif={createNotif} mode="buyer" /></div>}
        {tab === "profil" && <div className="buyer-page-card"><BuyerProfile user={user} profile={profile} /></div>}
      </main>
    </div>
  );
}
...
function BuyerAdCarousel({ banners = [], onExplore }) {
  const slides = Array.isArray(banners) ? banners.filter((item) => item?.imageUrl).slice(0, 5) : [];
  if (slides.length === 0) return null;

  return (
    <section className="buyer-ad-carousel-card" aria-label="Banner iklan buyer">
      <div className="buyer-ad-carousel-head">
        <div>
          <b>Promo & Info UMKM</b>
          <span>Geser untuk melihat banner lainnya</span>
        </div>
        <button type="button" onClick={onExplore}>Belanja</button>
      </div>
      <div className="buyer-ad-carousel-track">
        {slides.map((banner, index) => (
          <div className="buyer-ad-slide" key={banner.id || banner.imageUrl || index}>
            <img src={banner.imageUrl} alt={banner.title || `Banner promo ${index + 1}`} loading="lazy" />
          </div>
        ))}
      </div>
      {slides.length > 1 && (
        <div className="buyer-ad-dots">
  ...
  async function requestCancelOrder() {
    ...
    try {
      if (alreadyUploadedProof) {
        ...
        await restoreProductStockOnce({ ...order, statusPesanan: "dibatalkan", statusPembayaran: "dibatalkan" });
...
function BuyerProfile({ user, profile }) {
  const initialAddress = profile?.savedShippingAddress || {};
  const [editingAddress, setEditingAddress] = useState(false);
  const [savingAddress, setSavingAddress] = useState(false);
  const [addressMessage, setAddressMessage] = useState("");
  const [addressForm, setAddressForm] = useState({
    whatsapp: profile?.whatsapp || "",
    buyerAddress: initialAddress.buyerAddress || profile?.detailAddress || "",
    buyerVillage: initialAddress.buyerVillage || profile?.village || "",
    buyerDistrict: initialAddress.buyerDistrict || profile?.district || "",
    buyerRegency: initialAddress.buyerRegency || profile?.regency || "",
  });
  const displayFullAddress = [addressForm.buyerAddress, addressForm.buyerVillage, addressForm.buyerDistrict, addressForm.buyerRegency].filter(Boolean).join(", ");

  async function saveBuyerAddress(e) {
    e.preventDefault();
    if (!user?.uid) { setAddressMessage("Sesi login tidak valid. Silakan login ulang."); return; }
    if (!addressForm.buyerAddress.trim() || !addressForm.buyerVillage.trim() || !addressForm.buyerDistrict.trim() || !addressForm.buyerRegency.trim()) {
      setAddressMessage("Lengkapi detail alamat, desa, kecamatan, dan kabupaten dulu.");
      return;
    }
    setSavingAddress(true);
    setAddressMessage("");
    try {
      const fullAddress = [addressForm.buyerAddress, addressForm.buyerVillage, addressForm.buyerDistrict, addressForm.buyerRegency].filter(Boolean).join(", ");
      const savedShippingAddress = {
        buyerAddress: addressForm.buyerAddress.trim(),
        buyerVillage: addressForm.buyerVillage.trim(),
        buyerDistrict: addressForm.buyerDistrict.trim(),
        buyerRegency: addressForm.buyerRegency.trim(),
        buyerFullAddress: fullAddress,
        buyerMapsLink: profile?.savedShippingAddress?.buyerMapsLink || "",
        updatedAt: new Date().toISOString(),
      };
      await setDoc(doc(db, "users", user.uid), {
        whatsapp: addressForm.whatsapp.trim(),
        village: savedShippingAddress.buyerVillage,
        district: savedShippingAddress.buyerDistrict,
        regency: savedShippingAddress.buyerRegency,
        detailAddress: savedShippingAddress.buyerAddress,
        fullAddress,
        savedShippingAddress,
        updatedAt: serverTimestamp(),
      }, { merge: true });
      try { localStorage.setItem(`umkm_last_shipping_address_${user.uid}`, JSON.stringify(savedShippingAddress)); } catch {}
      setAddressMessage("Alamat berhasil disimpan. Checkout berikutnya akan otomatis memakai alamat terbaru ini.");
      setEditingAddress(false);
    } catch (error) {
      console.error("Gagal menyimpan alamat buyer:", error);
      setAddressMessage("Gagal menyimpan alamat. Coba lagi.");
    } finally {
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: 560 }}>
        <div style={{ display: "flex", gap: 16, alignItems: "center", marginBottom: 20 }}>
          <div style={{ width: 64, height: 64, borderRadius: "50%", background: "var(--orange-light)", color: "var(--orange)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, fontWeight: 700 }}>{profile?.name?.[0]?.toUpperCase() || "B"}</div>
          <div>
        ...
        {[["Email", profile?.email], ["WhatsApp", profile?.whatsapp || "-"], ["Status Akun", profile?.status === "active" ? "✅ Aktif" : profile?.status]].map(([l,v]) => (
          <div key={l} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "10px 0", borderBottom: "1px solid var(--border)", fontSize: 14 }}>
            <span style={{ color: "var(--text2)" }}>{l}</span>
            <span style={{ fontWeight: 500, textAlign: "right" }}>{v}</span>
          </div>
        ))}

        <div style={{ marginTop: 18, padding: 14, borderRadius: 14, background: "#FFF8E1", border: "1px solid rgba(245, 158, 11, 0.22)" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 10 }}>
            <div>
              <div style={{ fontWeight: 800, color: "var(--text1)" }}>📍 Alamat Pengiriman</div>
              <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>Alamat ini otomatis terisi saat checkout/order.</div>
            </div>
            <button type="button" className="btn-outline btn-sm" onClick={() => { setAddressMessage(""); setEditingAddress((v) => !v); }}>
              {editingAddress ? "Batal" : "Edit Alamat"}
            </button>
          </div>

          {!editingAddress ? (
            <div style={{ display: "grid", gap: 8, fontSize: 14 }}>
              <div><b>Detail:</b> {addressForm.buyerAddress || "Belum diisi"}</div>
              <div><b>Desa/Kelurahan:</b> {addressForm.buyerVillage || "Belum diisi"}</div>
              <div><b>Kecamatan:</b> {addressForm.buyerDistrict || "Belum diisi"}</div>
              <div><b>Kabupaten/Kota:</b> {addressForm.buyerRegency || "Belum diisi"}</div>
              <div style={{ color: "var(--text2)", fontSize: 13 }}><b>Alamat lengkap:</b> {displayFullAddress || "Belum ada alamat tersimpan"}</div>
            </div>
          ) : (
            <form onSubmit={saveBuyerAddress} style={{ display: "grid", gap: 10 }}>
              <div className="form-group"><label>WhatsApp</label><input className="form-input" value={addressForm.whatsapp} onChange={(e) => setAddressForm({ ...addressForm, whatsapp: e.target.value })} placeholder="08xxxxxxxxxx" /></div>
              <div className="form-group"><label>Detail Alamat</label><textarea className="form-input" rows={2} value={addressForm.buyerAddress} onChange={(e) => setAddressForm({ ...addressForm, buyerAddress: e.target.value })} placeholder="Kampung/Jalan/RT/RW/patokan" required /></div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8 }}>
                <input className="form-input" value={addressForm.buyerVillage} onChange={(e) => setAddressForm({ ...addressForm, buyerVillage: e.target.value })} placeholder="Desa/Kelurahan" required />
                <input className="form-input" value={addressForm.buyerDistrict} onChange={(e) => setAddressForm({ ...addressForm, buyerDistrict: e.target.value })} placeholder="Kecamatan" required />
                <input className="form-input" value={addressForm.buyerRegency} onChange={(e) => setAddressForm({ ...addressForm, buyerRegency: e.target.value })} placeholder="Kabupaten/Kota" required />
              </div>
              <button type="submit" className="btn-primary" disabled={savingAddress} style={{ justifyContent: "center" }}>{savingAddress ? "Menyimpan..." : "Simpan Alamat"}</button>
            </form>
          )}
          {addressMessage && <div style={{ marginTop: 10, fontSize: 12, color: addressMessage.includes("berhasil") ? "#10B981" : "#EF4444", fontWeight: 700 }}>{addressMessage}</div>}
        </div>
      </div>
    </div>
  );

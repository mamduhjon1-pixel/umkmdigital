...
function BuyerProfile({ user, profile }) {
  const savedAddress = profile?.savedShippingAddress || {};
  const [saving, setSaving] = useState(false);
  const [photoFile, setPhotoFile] = useState(null);
  const [form, setForm] = useState({
    name: profile?.name || "",
    username: profile?.username || profile?.buyerUsername || "",
    whatsapp: profile?.whatsapp || "",
    photoURL: profile?.photoURL || profile?.buyerPhotoURL || "",
    fullAddress: profile?.fullAddress || profile?.detailAddress || savedAddress.buyerAddress || "",
    mapsLink: profile?.mapsLink || savedAddress.buyerMapsLink || "",
    village: profile?.village || savedAddress.buyerVillage || "",
    district: profile?.district || savedAddress.buyerDistrict || "",
    regency: profile?.regency || savedAddress.buyerRegency || "",
    addressNote: profile?.addressNote || "",
  });

  async function saveProfile(e) {
    e.preventDefault();
    if (!user?.uid) { alert("Sesi login tidak valid. Silakan login ulang."); return; }
    setSaving(true);
    try {
      let photoURL = form.photoURL || "";
      if (photoFile) photoURL = await uploadImageToCloudinary(photoFile);
      const addressToSave = {
        buyerAddress: form.fullAddress || "",
        buyerMapsLink: form.mapsLink || "",
      ...
      await updateDoc(doc(db, "users", user.uid), {
        ...
        buyerUsername: form.username || "",
        whatsapp: form.whatsapp || "",
        photoURL,
        buyerPhotoURL: photoURL,
        fullAddress: form.fullAddress || "",
        detailAddress: form.fullAddress || "",
      ...
      setPhotoFile(null);
      alert("Profil buyer berhasil disimpan. Checkout berikutnya otomatis memakai data ini.");
    } catch (error) {
      console.error("Gagal menyimpan profil buyer:", error);
      alert(error?.message || "Gagal menyimpan profil buyer.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div>
      <div className="page-section-title">👤 Profil Saya</div>
      <form className="buyer-profile-card" onSubmit={saveProfile}>
        <div className="buyer-profile-head">
          <div className="buyer-profile-avatar">
            {form.photoURL ? <img src={form.photoURL} alt="Foto profil" /> : <span>{form.name?.[0]?.toUpperCase() || "B"}</span>}
          </div>
          <div>
            <div className="buyer-profile-name">{form.name || "Pembeli"}</div>
            <span className="badge badge-green">Pembeli Aktif</span>
            <input type="file" accept="image/*" onChange={(e) => setPhotoFile(e.target.files?.[0] || null)} style={{ marginTop: 10, fontSize: 12 }} />
          </div>
        </div>

        <div className="buyer-profile-grid">
          <label>Nama Lengkap<input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label>
          <label>Username<input className="form-input" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} placeholder="contoh: riorey" /></label>
          <label>Email<input className="form-input" value={profile?.email || user?.email || ""} disabled /></label>
          <label>WhatsApp<input className="form-input" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} required /></label>
          <label className="buyer-profile-wide">Alamat Lengkap<textarea className="form-input" rows={2} value={form.fullAddress} onChange={(e) => setForm({ ...form, fullAddress: e.target.value })} placeholder="Alamat lengkap untuk pengiriman" required /></label>
          <label>Desa<input className="form-input" value={form.village} onChange={(e) => setForm({ ...form, village: e.target.value })} /></label>
          <label>Kecamatan<input className="form-input" value={form.district} onChange={(e) => setForm({ ...form, district: e.target.value })} /></label>
          <label>Kabupaten<input className="form-input" value={form.regency} onChange={(e) => setForm({ ...form, regency: e.target.value })} /></label>
          <label className="buyer-profile-wide">Link Google Maps<input className="form-input" value={form.mapsLink} onChange={(e) => setForm({ ...form, mapsLink: e.target.value })} placeholder="https://maps.google.com/..." /></label>
          <label className="buyer-profile-wide">Catatan Alamat<textarea className="form-input" rows={2} value={form.addressNote} onChange={(e) => setForm({ ...form, addressNote: e.target.value })} placeholder="Contoh: rumah pagar biru, dekat masjid" /></label>
        </div>

        <div className="buyer-profile-note">Data ini akan otomatis mengisi checkout dan ikut tersimpan pada order baru supaya seller menerima informasi lengkap.</div>
        <button className="btn-primary" type="submit" disabled={saving}>{saving ? "Menyimpan..." : "Simpan Profil"}</button>
      </form>
    </div>
  );
}

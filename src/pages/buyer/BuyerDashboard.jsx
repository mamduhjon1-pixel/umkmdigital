...
/* ─── BUYER DASHBOARD ────────────────────────── */
export default function BuyerDashboard({ user, profile, orders, products, paymentSetting, createNotif, onAddToCart, onProductClick, setPage, activeTab = "beranda", onTabChange, notifications = [], chatUnread = 0, onLogout }) {
        ...
        <nav className="buyer-app-menu">
          {tabs.map((t) => (
            <button
              key={t.id}
              type="button"
              className={`buyer-app-menu-item ${t.isMarketplace ? "buyer-app-menu-center" : ""} ${tab === t.id ? "active" : ""}`}
        ...
        {tab === "beranda" && (
          <div className="buyer-home-page">
            ...
            <div className="buyer-quick-grid">
              ...
              <button type="button" onClick={() => goTab("chat")}><span>💬</span><b>Pesan</b><em>{chatUnread > 0 ? `${chatUnread} baru` : "Chat"}</em></button>
            </div>

...
const formatLocationOption = (value) => normalizeLocationText(value).toUpperCase();
const LOCATION_FIELD_ALIASES = {
  kabupaten: ["kabupaten", "regency", "buyerRegency", "sellerRegency", "storeRegency"],
  kecamatan: ["kecamatan", "district", "buyerDistrict", "sellerDistrict", "storeDistrict"],
  desa: ["desa", "village", "buyerVillage", "sellerVillage", "storeVillage"],
};
function getProductLocationValue(product, key) {
  const aliases = LOCATION_FIELD_ALIASES[key] || [key];
  for (const alias of aliases) {
    const direct = normalizeLocationText(product?.[alias]);
    if (direct) return direct;
    const sellerLocation = normalizeLocationText(product?.sellerLocation?.[alias] || product?.sellerLocation?.[key]);
    if (sellerLocation) return sellerLocation;
    const location = normalizeLocationText(product?.location?.[alias] || product?.location?.[key]);
    if (location) return location;
    const seller = normalizeLocationText(product?.seller?.[alias] || product?.seller?.[key]);
    if (seller) return seller;
  }
  return "";
}
function addUniqueLocation(map, value) {
  const cleaned = normalizeLocationText(value);
  if (!cleaned) return;
  const key = normalizeLocationKey(cleaned);
  if (!map.has(key)) map.set(key, formatLocationOption(cleaned));
}
...
function getLocationOptions(products = [], filter = emptyLocationFilter) {
  ...
  const desa = new Map();
  const selectedKabupaten = normalizeLocationKey(filter.kabupaten);
  const selectedKecamatan = normalizeLocationKey(filter.kecamatan);

  products.forEach((product) => {
    if (product?.isDeleted || product?.status !== "active") return;
    ...
    addUniqueLocation(kabupaten, kab);

    const matchesKabupaten = !selectedKabupaten || normalizeLocationKey(kab) === selectedKabupaten;
    const matchesKecamatan = !selectedKecamatan || normalizeLocationKey(kec) === selectedKecamatan;
    if (matchesKabupaten) addUniqueLocation(kecamatan, kec);
    if (matchesKabupaten && matchesKecamatan) addUniqueLocation(desa, des);
  });

  const sort = (map) => [...map.values()].sort((a, b) => a.localeCompare(b, "id", { sensitivity: "base" }));
...
function getCartStorageKey(userId = "guest") {
  return `umkm_cart_${userId || "guest"}`;
}
function loadCartFromStorage(userId = "guest") {
  try {
...
function saveCartToStorage(userId = "guest", items = []) {
  try { localStorage.setItem(getCartStorageKey(userId), JSON.stringify(Array.isArray(items) ? items : [])); } catch {}
}
...
function withTimeout(promise, ms = 3500, message = "Proses terlalu lama.") {
  return Promise.race([
    promise,
...
async function registerPushNotification(user, profile) {
  ...
  // Paling penting: suara realtime harus langsung aktif setelah user klik.
  // Ini tidak boleh menunggu service worker/database supaya tombol tidak macet di Mengaktifkan.
  ...
  if (typeof window !== "undefined" && "Notification" in window) {
    ...
    if (permission !== "granted") {
      permission = await withTimeout(
        ...
        8000,
        "Izin notifikasi terlalu lama. Coba klik lagi atau cek izin browser."
      );
    }
    if (permission !== "granted") {
      // Suara tetap aktif walaupun izin push browser belum diberikan.
      permission = "sound_only";
    }
  } else {
    permission = "sound_only";
  }

  // Service worker hanya tambahan untuk browser notification. Jangan biarkan ini membuat UI loading lama.
  if (navigator.serviceWorker) {
    try {
      await withTimeout(navigator.serviceWorker.register("/service-worker.js"), 3500, "Service worker timeout.");
      await withTimeout(navigator.serviceWorker.ready, 3500, "Service worker belum siap.");
    } catch (error) {
      console.warn("Service worker notification dilewati:", error);
    }
  }

  const localToken = `local_supabase_${user.uid}_${Date.now()}`;
  try { localStorage.setItem(PUSH_TOKEN_KEY, localToken); } catch {}

  // Simpan status ke Supabase secara non-blocking agar tombol tidak macet jika jaringan/RLS lambat.
  setDoc(doc(db, "users", user.uid), {
    notificationEnabled: true,
    notificationPermission: permission,
    notificationProvider: "supabase_realtime_sound",
    localNotificationToken: localToken,
    localNotificationUpdatedAt: serverTimestamp(),
    notificationDevice: {
    ...
    console.warn("Status notifikasi tidak tersimpan, tapi suara tetap aktif:", error);
  });
...
function getProductSearchHaystack(product = {}) {
  return normalizeSearchText([
    product.productName, product.name, product.sellerName, product.storeName,
    product.category, product.subCategory, product.description,
    product.desa, product.kecamatan, product.kabupaten,
    product.sellerLocation?.desa, product.sellerLocation?.kecamatan, product.sellerLocation?.kabupaten,
  ].filter(Boolean).join(" "));
}
...
function productSearchScore(product = {}, rawQuery = "") {
  ...
  if (!q) return 1;
  const name = normalizeSearchText(product.productName || product.name);
  const seller = normalizeSearchText(product.sellerName || product.storeName);
  const category = normalizeSearchText([product.category, product.subCategory].filter(Boolean).join(" "));
  const haystack = getProductSearchHaystack(product);
  const words = q.split(" ").filter(Boolean);
  let score = 0;
  if (name === q) score += 120;
  if (seller === q) score += 115;
  if (name.startsWith(q)) score += 90;
  if (seller.startsWith(q)) score += 88;
  if (name.includes(q)) score += 70;
  if (seller.includes(q)) score += 68;
  if (category.includes(q)) score += 35;
  if (haystack.includes(q)) score += 25;
  words.forEach((w) => {
    if (name.includes(w)) score += 14;
    if (seller.includes(w)) score += 13;
    if (category.includes(w)) score += 8;
    if (haystack.includes(w)) score += 4;
  });
  return score;
}

function filterProductsShopeeStyle(products = [], queryText = "") {
  const q = normalizeSearchText(queryText);
  const visible = products.filter((p) => p.status === "active" && !p.isDeleted);
  if (!q) return visible;
  return visible
    .map((product) => ({ product, score: productSearchScore(product, q) }))
    .filter((item) => item.score > 0)
    .sort((a, b) => b.score - a.score || productSoldCount(b.product) - productSoldCount(a.product) || String(a.product.productName || "").localeCompare(String(b.product.productName || ""), "id"))
...
function formatSellerCopyAddress(order = {}) {
  return [order.buyerVillage, order.buyerDistrict, order.buyerRegency, order.buyerPostalCode || order.postalCode]
    .map(formatUpper)
    .filter(Boolean)
    .join(", ");
}
...
function DashboardBadge({ value }) {
  return <span className="badge-count" style={{ position: "static", marginLeft: 6 }}>{Number(value || 0)}</span>;
}
...
function getOrderShippingAddress(order = {}) {
  return formatSellerCopyAddress(order);
}
...
export default function App() {
  ...
  const [page, setPage] = useState(() => {
    try {
      ...
      if (window.location.hash && /access_token|type=recovery|refresh_token/.test(window.location.hash)) return "update-password";
  ...
  async function enablePushNotifications() {
    ...
    try {
      ...
      setPushStatus(getNotificationPermission() === "granted" ? "granted" : "sound_only");
      setPushError("");
      alert("Notifikasi realtime aktif. Suara notifikasi sudah aktif; push browser aktif jika izin browser diberikan.");
    } catch (error) {
      console.error("Gagal mengaktifkan notifikasi:", error);
      // Fallback: tetap aktifkan suara realtime agar user tidak mentok.
      unlockNotificationSound();
      lockNotificationSession();
      setPushStatus("sound_only");
      setPushError("Suara realtime aktif. Push browser belum aktif: " + (error?.message || "izin browser belum diberikan."));
    } finally {
      setPushBusy(false);
    }
  }
  ...
  async function logoutAndClearSession() {
    ...
    const userId = logoutUser?.uid || logoutUser?.id;

    // UI keluar dulu agar logout terasa instan dan tidak loading lama.
    ...
    // Update offline + signOut jalan di belakang. Jangan blok UI.
    if (logoutUser) clearUserPresence(logoutUser, { reason: "logout" }).catch(() => {});
    if (userId) {
    ...
    signOut(auth).catch((error) => console.warn("Logout Supabase gagal:", error));
  }

  async function loadUserProfile(authUser) {
    if (!authUser) return { appUser: null, appProfile: null };

    const byAuthId = await getDoc(doc(db, "users", authUser.uid));
    if (byAuthId.exists()) {
      const profile = byAuthId.data();
      const appUid = profile.uid || byAuthId.id || authUser.uid;
      return {
        appUser: { ...authUser, uid: appUid, authUid: authUser.uid },
        appProfile: { ...profile, id: byAuthId.id, uid: appUid, authUid: authUser.uid },
      };
    }

    // Fallback ini mencari profil berdasarkan email agar role admin/seller/buyer tetap terbaca
    // tanpa mengubah flow dashboard lama yang bergantung pada user.uid.
    ...
    if (email) {
      ...
      if (emailDoc) {
        const profile = emailDoc.data();
        const appUid = profile.uid || emailDoc.id || authUser.uid;
        try {
          await setDoc(doc(db, "users", emailDoc.id), {
            uid: appUid,
          ...
          console.warn("Profil ditemukan, tetapi gagal sinkron authUid:", syncError);
        }
        return {
          appUser: { ...authUser, uid: appUid, authUid: authUser.uid },
          appProfile: { ...profile, id: emailDoc.id, uid: appUid, authUid: authUser.uid },
        };
      }
    }

    return {
      appUser: { ...authUser, uid: authUser.uid, authUid: authUser.uid },
      appProfile: null,
    };
  }
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (u) => {
      if (!alive) return;
      if (!u) {
        try { if (user?.uid) await clearUserPresence(user, { reason: "auth_signed_out" }); } catch {}
        setUser(null);
        setProfile(null);
        setLoading(false);
        return;
      ...
      // Tampilkan dashboard secepat mungkin setelah Supabase Auth memberi session.
      // Role dari user_metadata dipakai sebagai profil sementara agar login tidak menunggu query users.
      const fastUser = { ...u, uid: u.uid, authUid: u.uid };
      let storedRole = null;
      try {
        const emailKey = (u.email || u?.user_metadata?.email || "").toLowerCase();
        storedRole = sessionStorage.getItem("umkm_fast_login_role") || (emailKey ? localStorage.getItem(`umkm_role_${emailKey}`) : null);
      } catch {}
      const metadataRole = u?.user_metadata?.role || u?.app_metadata?.role || storedRole || null;
      const optimisticProfile = metadataRole ? {
        uid: u.uid,
        authUid: u.uid,
      ...
      setUser(fastUser);
      if (optimisticProfile) setProfile(optimisticProfile);
      setLoading(false);

      try {
        const shouldFastRedirect = sessionStorage.getItem("umkm_after_login_dashboard") === "1";
        if (shouldFastRedirect && metadataRole) {
          const fastPage = getDashboardPageForRole(metadataRole);
          if (fastPage && fastPage !== page) navGoTo(fastPage);
        }
      } catch {}

      try {
        const { appUser, appProfile } = await loadUserProfile(u);
        if (!alive) return;
        setUser(appUser);
        const safeProfile = appProfile || optimisticProfile || {
          uid: appUser?.uid || u.uid,
          authUid: u.uid,
          email: u.email || "",
          name: u?.user_metadata?.name || u.email || "User",
          role: metadataRole || "buyer",
          _fallback: true,
        };
        setProfile(safeProfile);
        try {
          if (safeProfile?.role && u?.email) localStorage.setItem(`umkm_role_${String(u.email).toLowerCase()}`, safeProfile.role);
        } catch {}
        try { await setUserPresence(appUser, safeProfile, { reason: "auth_ready" }); } catch (presenceError) { console.warn("Gagal update presence:", presenceError); }
      } catch (error) {
        console.error("Gagal memuat profil user:", error);
        if (!alive) return;
        const fallbackProfile = optimisticProfile || {
          uid: fastUser.uid,
          authUid: fastUser.authUid,
          email: u.email || "",
          name: u?.user_metadata?.name || u.email || "User",
          role: metadataRole || "buyer",
          _fallback: true,
  ...
  const shouldLoadProducts = page === "home" || page === "sellerStore" || page === "buyer" || page === "seller" || page === "admin" || showCart || showCheckout || !!selectedProduct;
  const shouldLoadOrders = !!user && !!profile && (page === "buyer" || page === "seller" || page === "admin");
  const shouldLoadReviews = page === "home" || page === "sellerStore" || !!selectedProduct;
  const shouldLoadSellerFinance = !!user && !!profile && (page === "seller" || page === "admin");
  const shouldLoadAdminHeavyData = !!user && !!profile && isAdminRole && page === "admin";

  useEffect(() => {
    if (!shouldLoadProducts) return;
    const productsRef = page === "seller" && user?.uid
      ? query(collection(db, "products"), where("sellerId", "==", user.uid), limit(300))
      : page === "admin"
    ...
    return () => unsub();
  }, [shouldLoadProducts, page, user?.uid]);

  useEffect(() => {
    if (!shouldLoadOrders) return;
    const ordersRef = page === "buyer" && user?.uid
      ? query(collection(db, "orders"), where("buyerId", "==", user.uid), limit(300))
      : page === "seller" && user?.uid
        ? query(collection(db, "orders"), where("sellerId", "==", user.uid), limit(300))
    ...
    return () => unsub();
  }, [shouldLoadOrders, page, user?.uid]);

  useEffect(() => {
    if (!shouldLoadReviews) return;
    const unsub = onSnapshot(query(collection(db, "reviews"), limit(300)), (snap) => {
  ...
  useEffect(() => {
    if (!shouldLoadSellerFinance) return;
    const withdrawalsRef = page === "seller" && user?.uid
      ? query(collection(db, "withdrawals"), where("sellerId", "==", user.uid), limit(200))
      : query(collection(db, "withdrawals"), limit(300));
    const unsub = onSnapshot(withdrawalsRef, (snap) => {
    ...
    return () => unsub();
  }, [shouldLoadSellerFinance, page, user?.uid]);

  useEffect(() => {
    if (!shouldLoadSellerFinance) return;
    const walletsRef = page === "seller" && user?.uid
      ? query(collection(db, "seller_wallets"), where("sellerId", "==", user.uid), limit(20))
      : query(collection(db, "seller_wallets"), limit(300));
    const unsub = onSnapshot(walletsRef, (snap) => {
  ...
  useEffect(() => {
    if (!shouldLoadSellerFinance) return;
    const commissionBillsRef = page === "seller" && user?.uid
      ? query(collection(db, "komisi_tagihan"), where("sellerId", "==", user.uid), limit(200))
      : query(collection(db, "komisi_tagihan"), limit(300));
    const unsub = onSnapshot(commissionBillsRef, (snap) => {
  ...
  useEffect(() => {
    if (!shouldLoadAdminHeavyData) return;
    ...
    return () => unsub();
  }, [shouldLoadAdminHeavyData]);

  useEffect(() => {
    if (!shouldLoadAdminHeavyData) return;
    const unsub = onSnapshot(query(collection(db, "admin_commission_transactions"), limit(300)), (snap) => {
      setAdminCommissionTransactions(sortNewest(snap.docs.map((d) => ({ id: d.id, ...d.data() }))));
    }, (error) => {
      console.error("Admin commission transactions error:", error);
  ...
  useEffect(() => {
    if (!shouldLoadAdminHeavyData) return;
    const unsub = onSnapshot(query(collection(db, "user_presence"), limit(500)), (snap) => {
      setUserPresenceState(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
    }, (error) => {
  ...
  useEffect(() => {
    ...
    const updateOnline = () => {
      if (stopped) return;
      setUserPresence(user, profile || {}, { currentPage: page, visibilityState: document.visibilityState }).catch((error) => console.warn("Gagal heartbeat presence:", error));
    };
    updateOnline();
    presenceHeartbeatRef.current = setInterval(updateOnline, 30000);
    const handleVisibility = () => updateOnline();
    const handleBeforeUnload = () => {
      try {
        clearUserPresence(user, { reason: "page_close" });
      } catch {}
    };
    ...
    return () => {
      ...
      clearUserPresence(user, { reason: "component_unmount" }).catch(() => {});
    };
  }, [user?.uid, profile?.role, page]);
  ...
  useEffect(() => {
    ...
    try {
      shouldRedirect = sessionStorage.getItem("umkm_after_login_dashboard") === "1";
      if (shouldRedirect) sessionStorage.removeItem("umkm_after_login_dashboard");
    } catch {}
    if (!shouldRedirect) return;
    const nextPage = getDashboardPageForRole(profile.role);
  ...
  useEffect(() => {
    ...
    const qNotif =
      profile.role === "admin" || profile.role === "sub_admin"
        ? query(collection(db, "notifications"), where("role", "==", "admin"), limit(100))
        : query(collection(db, "notifications"), where("userId", "==", user.uid), limit(100));
  ...
  useEffect(() => {
    ...
    cartRealtimeReadyRef.current = false;
    setCart(loadCartFromStorage(userId));
    setSelectedCartIds([]);

    if (!user?.uid) {
      cartRealtimeReadyRef.current = true;
      return;
    }

    const unsub = onSnapshot(doc(db, "carts", user.uid), (snap) => {
      const remoteItems = snap.exists() ? normalizeCartItems(snap.data()?.items || []) : loadCartFromStorage(user.uid);
      lastSavedCartJsonRef.current = JSON.stringify(remoteItems);
      setCart(remoteItems);
      setSelectedCartIds((prev) => prev.filter((id) => remoteItems.some((item) => item.id === id)));
      cartRealtimeReadyRef.current = true;
    }, (error) => {
      console.warn("Realtime cart belum aktif atau table carts belum dibuat. Pakai cart lokal dulu:", error?.message || error);
      cartRealtimeReadyRef.current = true;
    });
    return () => unsub();
  }, [user?.uid]);
  ...
  useEffect(() => {
    ...
    saveCartToStorage(userId, normalized);
    if (!cartRealtimeReadyRef.current || !user?.uid) return;

    const json = JSON.stringify(normalized);
    if (json === lastSavedCartJsonRef.current) return;
    lastSavedCartJsonRef.current = json;
    setDoc(doc(db, "carts", user.uid), {
      userId: user.uid,
      items: normalized,
      updatedAt: serverTimestamp(),
    }, { merge: true }).catch((error) => {
      console.warn("Gagal sync cart realtime, cart lokal tetap tersimpan:", error?.message || error);
    });
  }, [cart, user?.uid]);
  ...
  const protectedPages = new Set(["buyer", "seller", "admin", "notif", "chat"]);
  const roleDashboardPages = new Set(["buyer", "seller", "admin"]);
  const isProtectedPage = protectedPages.has(page);
  const isRoleDashboardPage = roleDashboardPages.has(page);
  const profileStillLoading = !!user && !profile && isProtectedPage;

  if (loading || profileStillLoading) {
    return (
      <div className="loading-screen">
        <div className="spinner" />
        <p style={{ color: "#999", fontSize: 14 }}>{profileStillLoading ? "Memuat dashboard dan role akun..." : "Memuat aplikasi..."}</p>
      </div>
    );
  }
  ...
  function navGoTo(p) {
    ...
    try {
      ...
      if (p && p !== "home") url.searchParams.set("page", p);
  ...
  if (isProtectedPage && !user) {
    return (
      <div className="loading-screen" style={{ padding: 20, textAlign: "center" }}>
        <div style={{ fontSize: 42, marginBottom: 8 }}>🔐</div>
        <h2 style={{ margin: 0, color: "var(--text)" }}>Silakan login dulu</h2>
        <p style={{ color: "#777", maxWidth: 420 }}>Halaman ini membutuhkan session login. Setelah login, dashboard akan terbuka sesuai role akun.</p>
        <button className="btn-primary" onClick={() => navGoTo("login")}>Masuk</button>
      </div>
    );
  }

  if (isRoleDashboardPage && user && !hasDashboardAccess) {
    const correctPage = getDashboardPageForRole(profile?.role);
    return (
      <div className="loading-screen" style={{ padding: 20, textAlign: "center" }}>
        <div style={{ fontSize: 42, marginBottom: 8 }}>⚠️</div>
        <h2 style={{ margin: 0, color: "var(--text)" }}>Dashboard tidak sesuai role akun</h2>
        <p style={{ color: "#777", maxWidth: 460 }}>Akun ini terdeteksi sebagai <b>{profile?.role || "user"}</b>, jadi halaman <b>{page}</b> tidak dibuka.</p>
        <button className="btn-primary" onClick={() => navGoTo(correctPage)}>Buka Dashboard Saya</button>
      </div>
    );
  }
...
/* ─── HOME PAGE ─────────────────────────────── */
function HomePage({ products, search, rawSearch = "", onProductClick, onAddToCart, user, profile, setPage, locationFilter = emptyLocationFilter, onLocationFilterChange, onResetLocationFilter }) {
  const [activeCategory, setActiveCategory] = useState("all");
  const [activeSubCategory, setActiveSubCategory] = useState("all");
  const [sortBy, setSortBy] = useState("terbaru");

  const normalizedLocationFilter = useMemo(() => ({
    kabupaten: formatLocationOption(locationFilter.kabupaten),
    kecamatan: formatLocationOption(locationFilter.kecamatan),
    desa: formatLocationOption(locationFilter.desa),
  }), [locationFilter.kabupaten, locationFilter.kecamatan, locationFilter.desa]);
  const locationOptions = useMemo(() => getLocationOptions(products, normalizedLocationFilter), [products, normalizedLocationFilter.kabupaten, normalizedLocationFilter.kecamatan, normalizedLocationFilter.desa]);
  const hasLocationFilter = Boolean(normalizedLocationFilter.kabupaten || normalizedLocationFilter.kecamatan || normalizedLocationFilter.desa);
  const filtered = useMemo(() => {
    let result = filterProductsShopeeStyle(products, search);
    if (hasLocationFilter) result = result.filter((p) => productMatchesLocation(p, normalizedLocationFilter));
    if (activeCategory !== "all") result = result.filter((p) => p.category === activeCategory);
    if (activeSubCategory !== "all") result = result.filter((p) => p.subCategory === activeSubCategory);
    if (sortBy === "termurah") result = [...result].sort((a, b) => Number(a.price || 0) - Number(b.price || 0));
    if (sortBy === "termahal") result = [...result].sort((a, b) => Number(b.price || 0) - Number(a.price || 0));
    if (sortBy === "terlaris") result = [...result].sort((a, b) => productSoldCount(b) - productSoldCount(a));
    return result;
  }, [products, search, hasLocationFilter, normalizedLocationFilter.kabupaten, normalizedLocationFilter.kecamatan, normalizedLocationFilter.desa, activeCategory, activeSubCategory, sortBy]);
...
function LoginPage({ setPage }) {
  ...
  async function login(e) {
    ...
    try {
      try { sessionStorage.setItem("umkm_after_login_dashboard", "1"); } catch {}
      const result = await signInWithEmailAndPassword(auth, email.trim(), password);
      const authUser = result?.user || {};
      let role = authUser?.user_metadata?.role || authUser?.app_metadata?.role || "";

      // Fast path: kalau metadata role belum ada, ambil role akun langsung dari table users.
      // Ini hanya query kecil untuk 1 user, bukan menunggu semua data dashboard.
      if (!role) {
        try {
          const byAuthId = await getDoc(doc(db, "users", authUser.uid));
          if (byAuthId.exists()) role = byAuthId.data()?.role || "";
        } catch {}
      }
      if (!role) {
        try {
          const emailSnap = await getDocs(query(collection(db, "users"), where("email", "==", email.trim()), limit(1)));
          const emailDoc = emailSnap.docs[0];
          if (emailDoc) role = emailDoc.data()?.role || "";
        } catch {}
      }
      if (!role) role = "buyer";
      try {
        sessionStorage.setItem("umkm_fast_login_role", role);
        localStorage.setItem(`umkm_role_${email.trim().toLowerCase()}`, role);
      } catch {}

      if (role === "admin" || role === "sub_admin") setPage("admin");
...
function UpdatePasswordPage({ setPage }) {
  const [password, setPassword] = useState("");
  ...
  async function submitNewPassword(e) {
    ...
    try {
      ...
      setSuccess("Password berhasil diganti. Silakan login menggunakan password baru.");
      try { window.history.replaceState({}, document.title, window.location.origin); } catch {}
      setTimeout(() => setPage("login"), 1200);
    } catch (err) {
      setError(err.message || "Gagal mengganti password. Buka ulang link reset password dari email.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="auth-container" style={{ minHeight: "calc(100vh - 110px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ width: "100%", maxWidth: 420 }}>
        <div className="card form-card-mobile" style={{ padding: 36 }}>
          <div style={{ textAlign: "center", marginBottom: 28 }}>
            <div style={{ fontSize: 28, fontWeight: 800, color: "var(--orange)", marginBottom: 6 }}>UMKM Digital</div>
            <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>Buat Password Baru</div>
            <p style={{ fontSize: 13, color: "var(--text3)" }}>Masukkan password baru untuk akun Anda</p>
          </div>
          {error && <div style={{ background: "#FEE8E8", color: "#EF4444", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{error}</div>}
          {success && <div style={{ background: "#ECFDF5", color: "#047857", padding: "10px 14px", borderRadius: 8, marginBottom: 16, fontSize: 13 }}>{success}</div>}
          <form onSubmit={submitNewPassword} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div className="form-group">
              ...
              <input className="form-input" type="password" placeholder="Ulangi password baru" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
            </div>
            <button className="btn-primary" style={{ width: "100%", justifyContent: "center", padding: 13, fontSize: 15 }} disabled={loading}>
              {loading ? "Menyimpan..." : "Simpan Password Baru"}
            </button>
          </form>
          <button onClick={() => setPage("login")} style={{ background: "none", border: "none", color: "var(--orange)", fontSize: 13, cursor: "pointer", marginTop: 12, display: "block", textAlign: "center", width: "100%" }}>
            Kembali ke login
          </button>
        </div>
      </div>
...
function RegisterPage({ setPage, createNotif }) {
  ...
  async function register(e) {
    ...
    setLoading(true); setError("");
    try {
      if (form.role === "buyer" && !isValidPostalCode(form.postalCode)) throw new Error("Buyer wajib mengisi kode pos 5 digit.");
      const fullAddress = [form.detailAddress, form.village, form.district, form.regency, form.postalCode].filter(Boolean).join(", ");
      ...
      const res = await createUserWithEmailAndPassword(auth, form.email, form.password, { data: profilePayload });
      const finalProfile = { ...profilePayload, uid: res.user.uid, createdAt: serverTimestamp() };
      try {
        await setDoc(doc(db, "users", res.user.uid), finalProfile);
      } catch (profileError) {
        // Jika email confirmation Supabase aktif, user belum punya session sehingga RLS menolak write dari client.
        // Trigger SQL auth.users akan tetap membuat profile dari metadata.
        if (!res.needsEmailConfirmation) throw profileError;
        console.warn("Profile dibuat oleh trigger Supabase setelah email confirmation:", profileError);
      }
      ...
      if (form.role === "seller") {
        try {
          ...
          if (!res.needsEmailConfirmation) throw walletError;
          console.warn("Wallet seller dibuat oleh trigger Supabase setelah email confirmation:", walletError);
        }
        if (!res.needsEmailConfirmation) await createNotif({ role: "admin", type: "seller_register", title: "Pendaftaran Seller Baru", message: `${form.name} mendaftar sebagai seller baru. Menunggu persetujuan.` });
      } else {
        if (!res.needsEmailConfirmation) await createNotif({ role: "admin", type: "user_register", title: "Pengguna Baru Mendaftar", message: `${form.name} baru saja membuat akun sebagai pembeli.` });
      }
      if (res.needsEmailConfirmation) {
        alert("Pendaftaran berhasil. Jika verifikasi email masih aktif di Supabase, cek email untuk konfirmasi. Jika sudah dimatikan, akun bisa langsung login.");
      } else {
        alert("Pendaftaran berhasil. Akun bisa langsung digunakan untuk login.");
      }
      setPage("home");
    } catch (err) {
      ...
      <div style={{ width: "100%", maxWidth: 440 }}>
        <div className="card form-card-mobile" style={{ padding: 36 }}>
          ...
          <form onSubmit={register} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="form-group"><label>Kode Pos {form.role === "buyer" ? "(wajib buyer)" : ""}</label><input className="form-input" inputMode="numeric" maxLength={5} placeholder="Contoh 43179" value={form.postalCode} onChange={(e) => setForm({ ...form, postalCode: e.target.value.replace(/\D/g, "").slice(0, 5) })} required={form.role === "buyer"} /></div>
...
function CheckoutModal({ cart, user, profile, onClose, onSuccess, createNotif }) {
  ...
  const [form, setForm] = useState({
    ...
    buyerPostalCode: savedAddress.buyerPostalCode || savedAddress.postalCode || profile?.postalCode || "",
  });
  const [loading, setLoading] = useState(false);
  ...
  async function handleCheckout(e) {
    ...
    try {
      ...
      if (needsAddress && !isValidPostalCode(form.buyerPostalCode)) throw new Error("Kode pos wajib diisi 5 digit untuk alamat ekspedisi.");
      ...
      if (needsAddress || form.shippingType === "same_day") {
        ...
        try {
          await setDoc(doc(db, "users", user.uid), {
            savedShippingAddress: addressToSave,
            detailAddress: form.buyerAddress || "",
            village: form.buyerVillage || "",
            district: form.buyerDistrict || "",
            regency: form.buyerRegency || "",
            postalCode: form.buyerPostalCode || "",
            fullAddress: [form.buyerAddress, form.buyerVillage, form.buyerDistrict, form.buyerRegency, form.buyerPostalCode].filter(Boolean).join(", "),
          }, { merge: true });
        } catch (error) {
      ...
      for (const rawItem of cart) {
        ...
        const orderPayload = {
          ...
          buyerPostalCode: String(form.buyerPostalCode || ""),
          postalCode: String(form.buyerPostalCode || ""),
  ...
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal" style={{ maxWidth: 600 }} onClick={(e) => e.stopPropagation()}>
      ...
      <form onSubmit={handleCheckout}><div className="modal-body checkout-body">
        ...
        {needsAddress && <div style={{ background: "#FFF8E1", padding: 12, borderRadius: 8 }}><div style={{ fontWeight: 700, marginBottom: 8 }}>Alamat untuk cek ongkir</div><div style={{ fontSize: 12, color: "var(--text3)", marginBottom: 8 }}>Alamat ekspedisi akan otomatis disimpan ke profil dan terisi saat belanja berikutnya.</div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 8 }}><input className="form-input" placeholder="Desa" value={form.buyerVillage} onChange={(e) => { setForm({ ...form, buyerVillage: e.target.value }); setShippingCheckRequested(false); }} required /><input className="form-input" placeholder="Kecamatan" value={form.buyerDistrict} onChange={(e) => { setForm({ ...form, buyerDistrict: e.target.value }); setShippingCheckRequested(false); }} required /><input className="form-input" placeholder="Kabupaten" value={form.buyerRegency} onChange={(e) => { setForm({ ...form, buyerRegency: e.target.value }); setShippingCheckRequested(false); }} required /><input className="form-input" inputMode="numeric" maxLength={5} placeholder="Kode pos wajib" value={form.buyerPostalCode} onChange={(e) => { setForm({ ...form, buyerPostalCode: e.target.value.replace(/\D/g, "").slice(0, 5) }); setShippingCheckRequested(false); }} required /></div><button type="button" className="btn-primary btn-sm" style={{ marginTop: 10 }} onClick={() => { if (!form.buyerVillage || !form.buyerDistrict || !form.buyerRegency) { setError("Isi desa, kecamatan, dan kabupaten dulu."); return; } if (!isValidPostalCode(form.buyerPostalCode)) { setError("Kode pos wajib 5 digit."); return; } setShippingCheckRequested(true); setError(""); }}>Cek Ongkir</button><div style={{ fontSize: 12, color: shippingCheckRequested ? "#10B981" : "var(--orange)", marginTop: 6 }}>{shippingCheckRequested ? "Permintaan cek ongkir siap dikirim ke seller. Klik Buat Pesanan." : "Klik Cek Ongkir dulu agar seller mendapat perintah cek ongkir."}</div></div>}
      ...
      <div className="dash-sidebar">
        ...
        {tabs.map((t) => (
          <div key={t.id} className={`dash-sidebar-item ${tab === t.id ? "active" : ""}`} onClick={() => setTab(t.id)}>
            <span>{t.icon}</span> {t.label}{t.id === "pesanan" && <DashboardBadge value={buyerActiveOrderCount} />}
          </div>
        ))}
          ...
          {order.paymentMethod === "qris" ? (
            <div style={{ background: "#FFF8E1", borderRadius: 10, padding: 14, marginBottom: 12, fontSize: 13, border: "1px solid #FDE68A" }}>
              ...
              {paymentSetting?.qrisUrl ? (
                <img src={paymentSetting.qrisUrl} alt="QRIS Admin" onClick={() => openImagePreview(paymentSetting.qrisUrl, "QRIS Admin")} style={{ width: 220, maxWidth: "100%", borderRadius: 12, border: "1px solid var(--border)", background: "#fff", cursor: "zoom-in" }} />
              ) : (
...
function EditableProfileCard({ profile, title = "Profil Saya", activeLabel = "Aktif" }) {
  const [editing, setEditing] = useState(false);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    name: profile?.name || "",
    whatsapp: profile?.whatsapp || "",
    detailAddress: profile?.detailAddress || profile?.savedShippingAddress?.buyerAddress || "",
    village: profile?.village || profile?.savedShippingAddress?.buyerVillage || "",
    district: profile?.district || profile?.savedShippingAddress?.buyerDistrict || "",
    regency: profile?.regency || profile?.savedShippingAddress?.buyerRegency || "",
    postalCode: profile?.postalCode || profile?.savedShippingAddress?.buyerPostalCode || profile?.savedShippingAddress?.postalCode || "",
  });
  ...
  async function saveProfile(e) {
    e.preventDefault();
    if (!profile?.uid && !profile?.id) return alert("Profil belum terbaca. Login ulang lalu coba lagi.");
    if (profile?.role === "buyer" && !isValidPostalCode(form.postalCode)) return alert("Buyer wajib mengisi kode pos 5 digit.");
    setSaving(true);
    const savedShippingAddress = {
      buyerAddress: form.detailAddress || "",
      buyerVillage: form.village || "",
      ...
      buyerPostalCode: form.postalCode || "",
      postalCode: form.postalCode || "",
      buyerFullAddress: [form.detailAddress, form.village, form.district, form.regency, form.postalCode].filter(Boolean).join(", "),
      updatedAt: new Date().toISOString(),
    };
    try {
      await setDoc(doc(db, "users", profile.uid || profile.id), {
        name: form.name,
        whatsapp: form.whatsapp,
        detailAddress: form.detailAddress,
        village: form.village,
        ...
        regency: form.regency,
        postalCode: form.postalCode,
        fullAddress: savedShippingAddress.buyerFullAddress,
        savedShippingAddress,
        updatedAt: serverTimestamp(),
      }, { merge: true });
      try { localStorage.setItem(`umkm_last_shipping_address_${profile.uid || profile.id}`, JSON.stringify(savedShippingAddress)); } catch {}
      setEditing(false);
      alert("Profil dan alamat berhasil disimpan.");
    } catch (err) {
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: 560 }}>
        <div style={{ display: "flex", gap: 16, alignItems: "center", marginBottom: 20 }}>
          ...
          <button className="btn-ghost btn-sm" onClick={() => setEditing(!editing)}>{editing ? "Batal" : "Edit Profil"}</button>
        </div>
        {!editing ? (
          <>
            <div className="divider" />
            {[["Email", profile?.email],["WhatsApp", profile?.whatsapp || "-"],["Alamat Detail", profile?.detailAddress || "-"],["Wilayah", [profile?.village, profile?.district, profile?.regency, profile?.postalCode].filter(Boolean).join(", ") || "-"],["Status Akun", profile?.status === "active" ? "✅ Aktif" : profile?.status]].map(([l,v]) => (
              <div key={l} style={{ display: "flex", justifyContent: "space-between", gap: 14, padding: "10px 0", borderBottom: "1px solid var(--border)", fontSize: 14 }}>
          ...
          <form onSubmit={saveProfile} style={{ display: "grid", gap: 12 }}>
            <div className="form-group"><label>Nama</label><input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
            <div className="form-group"><label>WhatsApp</label><input className="form-input" value={form.whatsapp} onChange={(e) => setForm({ ...form, whatsapp: e.target.value })} required /></div>
            <div className="form-group"><label>Alamat Detail</label><textarea className="form-input" rows={2} value={form.detailAddress} onChange={(e) => setForm({ ...form, detailAddress: e.target.value })} required /></div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(130px, 1fr))", gap: 8 }}>
              <input className="form-input" placeholder="Desa" value={form.village} onChange={(e) => setForm({ ...form, village: e.target.value })} required />
              <input className="form-input" placeholder="Kecamatan" value={form.district} onChange={(e) => setForm({ ...form, district: e.target.value })} required />
              <input className="form-input" placeholder="Kabupaten" value={form.regency} onChange={(e) => setForm({ ...form, regency: e.target.value })} required />
              <input className="form-input" inputMode="numeric" maxLength={5} placeholder="Kode Pos" value={form.postalCode} onChange={(e) => setForm({ ...form, postalCode: e.target.value.replace(/\D/g, "").slice(0, 5) })} required={profile?.role === "buyer"} />
            </div>
...
function BuyerProfile({ profile }) {
  return <EditableProfileCard profile={profile} title="Profil Saya" activeLabel="Pembeli Aktif" />;
}
  ...
  return (
    <div className="dash-layout">
        ...
        {tabs.map((t) => {
          ...
          return (
            <div key={t.id} className={`dash-sidebar-item ${tab === t.id ? "active" : ""}`} onClick={() => blockedOrderTab ? alert("Pesanan masuk sedang diblokir admin. Selesaikan tagihan komisi lalu tunggu admin membuka blokir.") : setTab(t.id)} style={blockedOrderTab ? { opacity: 0.55, cursor: "not-allowed" } : undefined}>
              <span>{t.icon}</span> {t.label}{blockedOrderTab && <span className="badge badge-red" style={{ marginLeft: 6 }}>Diblokir</span>}{t.id === "order" && !blockedOrderTab && <DashboardBadge value={sellerIncomingOrderCount} />}{t.id === "chat" && <DashboardBadge value={chatUnread} />}{t.id === "produk" && <DashboardBadge value={products.filter((p) => !p.isDeleted).length} />}{t.id === "tagihan" && <DashboardBadge value={sellerCommissionBills.length} />}
            </div>
          );
        })}
        <div className="dash-logout-btn-wrap" style={{ padding: "8px 12px", marginTop: "auto" }}>
      ...
      <div className="dash-content">
        ...
        {tab === "profil" && (
          <div>
            <EditableProfileCard profile={profile} title="Profil Toko" activeLabel={profile?.status === "active" ? "Seller Aktif" : "Seller Pending"} />
            <div className="card" style={{ maxWidth: 560, marginTop: 14 }}>
              {[["Saldo Tersedia", rupiah(wallet?.saldoTersedia || 0)],["Total Penjualan", rupiah(sellerTotalSales)]].map(([l,v]) => (
            ...
            <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 8, padding: 10, background: "#F8FAFC", border: "1px solid var(--border)", borderRadius: 10 }}>
                  ...
                  {(o.buyerVillage || o.buyerDistrict || o.buyerRegency || o.buyerPostalCode) && <div>Wilayah: {[o.buyerVillage, o.buyerDistrict, o.buyerRegency, o.buyerPostalCode].filter(Boolean).join(", ")}</div>}
...
function AdminDashboard({ user, profile, products, orders, withdrawals, paymentSetting, manualBalance, commissionSetting, wallets, commissionBills = [], adminCommissionWallet, adminCommissionTransactions = [], users, userPresence = [], chatUnread = 0, createNotif, onLogout }) {
  ...
  const liveOnlineUsers = userPresence.filter((p) => p.isOnline || p.status === "online");
  const liveOnlineSellers = liveOnlineUsers.filter((p) => p.role === "seller");
      ...
      <div className="dash-content">
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 14, marginBottom: 24 }}>
          {[
            { label: "Total Produk", value: products.length, icon: "📦", color: "#EE4D2D" },
            { label: "Total Order", value: orders.length, icon: "🛒", color: "#3B82F6" },
            { label: "Online Live", value: liveOnlineUsers.length, icon: "🟢", color: "#10B981" },
            { label: "Seller Online", value: liveOnlineSellers.length, icon: "🏪", color: "#059669" },
...
async function approveSellerAccount(u) {
  if (!u || u.role !== "seller") return false;
  const sellerId = u.uid || u.id || u.authUid || u.userId;
  if (!sellerId) throw new Error("ID seller tidak ditemukan.");
  ...
  // Jalur utama: RPC Supabase security definer supaya approve tetap jalan meski RLS ketat.
  try {
    const { error } = await supabase.rpc("approve_seller", { p_seller_id: sellerId });
    if (!error) return true;
    console.warn("approve_seller RPC gagal, fallback ke updateDoc:", error);
  } catch (rpcError) {
    console.warn("approve_seller RPC belum tersedia, fallback ke updateDoc:", rpcError);
  }

  // Fallback: update langsung lewat policy RLS biasa.
  await updateDoc(doc(db, "users", sellerId), approvalPayload);

  // Sinkronkan juga jika ada row pengajuan seller dengan id/userId/sellerId yang sama.
  const appCandidates = [sellerId, u.userId, u.sellerId].filter(Boolean);
  for (const appId of appCandidates) {
    try {
      await updateDoc(doc(db, "seller_applications", appId), approvalPayload);
    } catch (_) {}
    try {
      await updateDoc(doc(db, "seller_verifications", appId), approvalPayload);
    } catch (_) {}
  }
  ...
  await setDoc(doc(db, "seller_wallets", sellerId), {
    ...
    saldoTersedia: Number(u.saldoTersedia || 0),
    saldoTertahan: Number(u.saldoTertahan || 0),
    totalPenjualan: Number(u.totalPenjualan || 0),
    totalDitarik: Number(u.totalDitarik || 0),
    updatedAt: serverTimestamp(),
  }, { merge: true });
...
function AdminSellerApprovals({ users = [] }) {
  ...
  async function approveSeller(u) {
    try {
      await approveSellerAccount(u);
      alert("Seller berhasil di-approve. Seller sekarang bisa upload produk.");
    } catch (error) {
      console.error("Gagal approve seller:", error);
      alert(error?.message || "Gagal approve seller. Pastikan SQL final terbaru sudah dijalankan dan akun admin punya role admin.");
    }
  }
      ...
      <div className="card" style={{ maxWidth: 520 }}>
        <form onSubmit={save} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="form-group">
            ...
            {qrisPreview && (
              <div style={{ marginTop: 10 }}>
                <img src={qrisPreview} alt="Preview QRIS" onClick={() => openImagePreview(qrisPreview, "Preview QRIS")} style={{ width: 220, maxWidth: "100%", borderRadius: 12, border: "1px solid var(--border)", background: "#fff", cursor: "zoom-in" }} />
              </div>
            )}
  ...
  async function submit(e) {
    ...
    try {
      ...
      try {
        ...
        if (!res.needsEmailConfirmation) throw profileError;
        console.warn("Profile sub admin dibuat oleh trigger Supabase setelah email confirmation:", profileError);
      }
...
function ChatCenter({ user, profile, createNotif, mode = "buyer", users = [] }) {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    const chatRef = isAdminChatMode
      ? query(collection(db, "chats"), where("chatType", "==", "admin_seller"), limit(300))
      : isSellerChatMode
        ? query(collection(db, "chats"), where("sellerId", "==", user.uid), limit(300))
        : query(collection(db, "chats"), where("buyerId", "==", user.uid), limit(300));
    ...
    return () => unsub();
  }, [user?.uid, mode, profile?.role, isAdminChatMode, isSellerChatMode]);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "chats", activeChat.id, "messages"), (snap) => {
      ...
      const incomingNew = data.filter((m) => m.senderId !== user.uid && !seenMessageIdsRef.current.has(m.id));
      if (!firstMsgLoadRef.current && incomingNew.length > 0) playOrderSound();
      seenMessageIdsRef.current = currentIds;
      firstMsgLoadRef.current = false;
      setMessages(data);

      incomingNew.concat(data.filter((m) => m.unreadFor === user.uid || (m.receiverId === user.uid && !m.isRead))).slice(0, 20).forEach((m) => {
        updateDoc(doc(db, "chats", activeChat.id, "messages", m.id), {
          isRead: true,
  ...
  function getOtherTypingName() {
    const typing = activeChat?.typing || {};
    const otherId = Object.keys(typing).find((id) => id !== user?.uid && typing[id]?.isTyping);
    if (!otherId) return "";
    const info = typing[otherId] || {};
    const ts = getMillis(info.updatedAt);
    if (ts && Date.now() - ts > 6000) return "";
    return info.name || getOtherName(activeChat);
  }
    ...
    return () => {
      ...
      if (activeChat?.id && user?.uid) {
        updateDoc(doc(db, "chats", activeChat.id), {
          typing: {
            ...(activeChat?.typing || {}),
            [user.uid]: { isTyping: false, name: profile?.name || "User", updatedAt: new Date().toISOString() },
          },
          updatedAt: serverTimestamp(),
        }).catch(() => {});
      }
        ...
        <div className="card chat-room-card" style={{ minHeight: 460, display: "flex", flexDirection: "column", padding: 0, overflow: "hidden" }}>
          {activeChat ? (
            <>
              ...
              {getOtherTypingName() && (
                <div style={{ padding: "6px 14px", fontSize: 12, color: "var(--text3)", borderTop: "1px solid var(--border)" }}>
                  {getOtherTypingName()} sedang mengetik...
                </div>
              )}
              <form className="chat-input-row" onSubmit={sendMessage} style={{ padding: 12, display: "flex", gap: 8, borderTop: "1px solid var(--border)" }}>
                <input className="form-input" value={text} onChange={(e) => handleTypingChange(e.target.value)} placeholder="Tulis pesan..." style={{ flex: 1 }} />
